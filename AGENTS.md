...
- Keep the current scope explicit. Roblox-style direction, touchscreen controls, keyboard/mouse, and Authentik-only sign-in are established in the brief; gamepad is a later option. Mechanics, exact device/browser coverage, engine, persistence, and player admission rules still need design. Do not turn deferred choices into settled requirements.
