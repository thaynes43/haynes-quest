...
This project is a novelty 3D browser game, initially for Tom Haynes's kids. Users configure a self-hosted photo-service URL, API key, and people; their photos drive an open-ended generated-character roster and game content. Immich is the first integration. The planned hosting target is the local cluster through `haynes-ops`.
...
- Image generation followed by Blender MCP is the preferred asset-authoring direction. Document the workflow now; defer connections, installations, assets, and prototype execution. Preserve the separate requirement for automatic per-user character generation in the deployed app.
