...
export function verifyToken(
  ...
  const match =
    /^([0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})\.([A-Za-z0-9_-]{43})$/i.exec(
      value,
    );
  ...
  take(key: string, now = Date.now()): void {
    ...
    if (!current || now - current.startedAt >= this.windowMs) {
      if (this.windows.size >= 2_000) {
        for (const [expiredKey, value] of this.windows) {
          if (now - value.startedAt >= this.windowMs)
            this.windows.delete(expiredKey);
        }
        if (!this.windows.has(key) && this.windows.size >= 2_000) {
    ...
    if (this.windows.size > 2_000) {
      for (const [keyToCheck, value] of this.windows) {
        if (now - value.startedAt >= this.windowMs)
          this.windows.delete(keyToCheck);
      }
    }
  }
}
