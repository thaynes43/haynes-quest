...
const analogActions = new Set<GameInputAction>(['moveX', 'moveY', 'lookX', 'lookY']);
...
export class GameInputState {
  ...
  set(action: GameInputAction, value: number | boolean): void {
    if (analogActions.has(action)) {
      this.external[action as 'moveX' | 'moveY' | 'lookX' | 'lookY'] = clampUnit(Number(value));
      return;
    }
    this.external[action as 'jump' | 'interact'] = Boolean(value);
  }
...
export function bindBrowserInput({ target, input }: BrowserInputBindingOptions): () => void {
