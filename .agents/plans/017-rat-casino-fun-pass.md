...
- Hosted verification from the home network, including any failed real-time attempt and its lockstep comparison.
- Feel on physical devices and the children's reactions, reported by Tom.

## Result

Slice 1 is implemented, deployed to the private playtest and verified locally and from the home network. The full real-time browser route remains open.
...
- **Full course:** the extended `tests/e2e/rat-casino.mjs` takes the golden-view side route and asserts the closing haul.
  ...
  - The other local attempts stopped at the seventh jump's takeoff. That is a limit of the route driver at about five frames a second, and it also stopped earlier runs on unmodified code. The hosted comparison is recorded below.
...
- **Browser checks on slow renderers (September 25):** two checks failed in the cloud container because of timing, not game faults.
  - `tests/e2e/casino-rewards.mjs` stepped its loft and fight frames without waiting for each to be drawn, so a screenshot timed out at the defeat. It now waits, as the lockstep route driver does. A local run then passed end to end in 27 minutes. It walked the foyer trail (2 tokens), made 11 loft jumps with no recovery to the golden ticket (20 tokens), and saw contact sparks 112 ms after the swing while the server's reply was held. The mascot fell in 2 swings with confetti, and the 390×844 tally fit. There were no page, console or network errors.
  - `tests/e2e/studio.mjs` read each animation clip once, 320 ms after starting it. Software WebGL can take most of a second to draw a newly loaded model's first frames, so two runs failed at different clips. It now polls for up to 3 s. A local run then passed all 12 pages, including the three sound pages PR67 changed, with no page, console or network errors.
- **Deployment:** app PR67 merged at `dc52e4b`. [Ops PR3169](https://github.com/thaynes43/haynes-ops/pull/3169) pinned its signed image to the private playtest; HelmRelease history shows revision 23 deployed at 2026-09-24 23:53 UTC, and the old ReplicaSet retains that image. The old pod's Ready state cannot be checked retrospectively.
  - Main `91a7c65`, after PR69, has the same application code; only tests and docs changed. [Ops PR3170](https://github.com/thaynes43/haynes-ops/pull/3170) pinned its signed image `sha256:b20e6a47…2103` to the playtest only. On September 25 the `frontend/haynes-quest` Kustomization was Ready at `cf1994f`; the playtest HelmRelease was Ready at generation 23/23 and revision 24. Pod `haynes-quest-playtest-5b59c747df-sk6jr` was Ready with zero restarts and the exact imageID digest. Normal Quest and dev-env kept their deployment UIDs, generations and images. No reconcile or restart was needed.
  - The hosted `/healthz`, `/readyz`, playtest guide, PLAN-017 studio page and runbook all returned 200; the runbook contained the real-time-and-lockstep section.
- **Hosted browser checks, September 25:** test harness `c30aab6` targeted the pinned `91a7c65` image through the home-network ingress. Chromium 153 used ANGLE's SwiftShader Vulkan software renderer; these figures are not physical-device performance.
  - Two full-course **real-time** attempts crossed the raised foyer, won the first two fights and reached the golden-view balcony. Both stopped before its ticket: `golden-ticket: target unreachable` at nearest distances 1.09 m and 1.15 m. Neither reached the boss, so there is no real-time boss-stage frame sample. Both reports recorded zero page, console, HTTP, failed-request and external-request errors. The full real-time gate did **not** pass.
  - A hosted lockstep comparison at scale 0.5 passed the complete course: deliberate fall and recovery, all five fights without retries, three memories, golden-view ticket, the closing haul (62/145 tokens, 1/3 tickets), editor sample, three-chapter garden start and 390×844 view with no horizontal overflow or browser/network errors. It used 1,277 stepped frames over 49.41 s of page time. The separate real-time garden sample measured 312.21 ms mean, 599.9 ms p95 and 3.2 fps on software rendering. Lockstep skipped the boss-stage frame sample and does not resolve real-time completion or feel.
  - `casino-rewards.mjs` passed: foyer trail 2 tokens, 11 loft jumps with no recovery to 20 tokens and its ticket, contact sparks 112 ms after the swing while the server reply was held, confirmed HP 4/8, defeat after two swings with 54 confetti particles, and a 390×844 tally without overflow. Error arrays were empty.
  - `studio.mjs` passed all 12 pages, nine models, seven clips, four decoded audio auditions, two touch orbits, 113 media files and seven manifests. The three fun-pass sound pages decoded; listening was not checked. Page, console, bad-response and external-request arrays were empty.
  - [The release record](../evidence/rat-casino-fun-pass-release.json) gives pod IDs, endpoint results, browser reports and SHA-256s. Raw reports and 54 screenshots remain outside git under `/home/dev/artifacts/haynes-quest/rat-casino-fun-pass-hosted-0925/`.
- **Open:** the full real-time browser journey still needs a passing run on suitable rendering hardware or a verified route-driver correction. Physical iPhone/iPad Safari play, feel and sound, the children's reactions and exact final-art review remain Tom's gates. The hosted lockstep result covers course logic only. Later DESIGN-022 ★ decisions remain with Tom.
