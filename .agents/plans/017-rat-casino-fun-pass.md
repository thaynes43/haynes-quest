...
- **Deployment:** app PR67 merged at `dc52e4b`.
  - [Ops PR3169](https://github.com/thaynes43/haynes-ops/pull/3169) pinned its signed image `sha256:e8a4aba0…7521` to the private playtest only and merged at `5a25d6f`. Flux reported that `cluster-apps` reconciled that commit at 23:53 UTC on September 24.
  - Main `91a7c65`, after PR69, has the same application code; only tests and docs changed. [Ops PR3170](https://github.com/thaynes43/haynes-ops/pull/3170) pinned its signed image `sha256:b20e6a47…2103` to the playtest only and merged at `3d149a5`. Its rendered diff changed only the playtest Deployment image. Flux reported that `cluster-apps` reconciled `cf1994f`, a later main commit that includes it, at 05:24 UTC on September 25. The commits in between touch no Quest files.
  - Those Flux statuses cover only the flux-system Kustomizations, not the playtest HelmRelease or pod.
  - The cloud session cannot reach the cluster or the home network, so neither pod rollout has been verified. [Issue 70](https://github.com/thaynes43/haynes-quest/issues/70) tracks the rollout and hosted checks. The [pending release record](../evidence/rat-casino-fun-pass-release.json) holds the image, GitOps and local evidence so far.
- **Open:** the playtest rollout check and the hosted checks from the home network, including a real-time run of the full course, both tracked in issue 70. Also open: physical-device feel and sound, and the children's reactions. Lockstep covers the course's logic locally but not these.
