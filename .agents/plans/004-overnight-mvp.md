...
OAuth client/redirect setup and a dev-env image change are not required tonight. Tom is preparing the minimal session/database fields in his own `haynes-quest` 1Password item now so their ExternalSecrets can be verified before handoff. The existing AppDaemon Immich key is reused through a separate ExternalSecret. Do not reuse another application’s session secret or database identity.
