...
Tom's September 11 correction supersedes unattended completion of a fixed enemy batch. Discuss brief encounter ideas and child interests before model production. The next selected design is The Besties duo; FNAF is a later interest to refine. Use one agreed concept, get feedback on its look, then proceed to the expensive model/rig/animation catalog. Pending decisions about the next cast do not block independent setup, gameplay, tests or ordinary checked PR work. Preserve and release current Blender work safely rather than running extra final renders to finish a superseded batch. See [DESIGN-012](../docs/designs/012-player-journey-curation.md).
