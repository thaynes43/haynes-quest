...
**Active September 12: [PLAN007](plans/007-playtest-reset-and-controls.md).** Tom reports the released candidate remains buggy on iPad/iPhone: artwork/save loop blocks Besties, grass/sound are not apparent, controls and motion are poor. Lead `/home/dev/work/quest-playtest-reset` on `agent/quest-playtest-reset`, base ed7ae72. Fable mobile-UX research WO062 owns the browser lease; native Sol fresh_sessions (backend), memory_route (v3 route contract), audible_feedback (audio module) own bounded lanes. Root owns UI/runtime/input/foliage, illustrations and integration. New work is authorized through checked private release; no dev-env restart, OAuth or generation. The PLAN006 historical delivery below remains evidence, not acceptance of the new feedback.

PLAN007 checkpoint: root commit `4307bd2` records the plan, DESIGN015, UI/input/grass and fixture art. Backend, v3 memory/secondary runtime, audio and catalog changes remain in the same shared worktree pending final tests and commit. Fable WO062 finished (`16c5d4f`, follow-up `bfadcd1`) and released its browser. Sol fresh_sessions now owns WO066 browser-harness work and the exclusive browser lease. Sol memory_route owns createGame/runtime tests through the current integration. Sol audible_feedback delivered audio and the 37-entry/54-thumbnail catalog; it is preparing a separate scoped operations worktree. Root owns all visible text/design and final review.

The preliminary built fixture is running at `http://127.0.0.1:4397`, root exec session90493, no database/Immich environment, ephemeral enabled. It currently serves model/audio assets from `/home/dev/work/quest-playtest-feedback/site`; root's new `site/` has also built successfully. Rebuild and restart only this owned fixture before final browser evidence. Unrelated4392 stays untouched. No PLAN007 PR or live rollout yet. No authoring job, OAuth or dev-env restart. Local cached WebKit fails its launch dependency check (missing GStreamer libraries), so browser evidence will be Chromium and physical Safari remains open.

The first phone capture is `test-results/fresh-playtest/root-first-phone.png`: visible grass and 88px Attack/48px header controls. It is a preliminary view, not full-route acceptance. Full tests are being reconciled with the deliberately changed artwork/timing behavior; archived physics defaults remain3.1m/s, v3 runtime opts into4m/s after the lead checked jump landing distances. The new exact fixture catalog integrity test passes. App typecheck/lint/build and strict docs have passed interim snapshots; rerun once runtime lane settles. Next: finish runtime/auto-amends/jump sequence tests, final build + full browser route and catalog, checked app PR merge, exact published image, scoped private GitOps flag/image PR merge, live validation and close records.
