...
Tom wants a novelty 3D browser game as a family PoC, with a configurable self-hosted photo library supplying game content and references for developer-authored characters. Immich is the first integration, with local hosting through `haynes-ops`. He chose `haynes-quest` on 2026-09-10.
...
After login, players choose a saved game or start a new one from their configured roster. Users supply the photo-service URL, API key, and people's names; the application resolves those people and retrieves gameplay imagery. The PoC uses authored models assigned to stable character records. There are no built-in personal names or fixed character count. Saves retain character IDs across name changes and model replacement.

[ADR-002](../docs/adrs/002-web-game-stack.md) proposes TypeScript, React/Vite, Babylon.js, Hono/Node, Better Auth, and Postgres/Drizzle. [DESIGN-001](../docs/designs/001-technical-foundation.md) covers saves/runtime; [DESIGN-002](../docs/designs/002-asset-pipeline.md) covers asset authoring; [DESIGN-003](../docs/designs/003-photo-connections-and-people.md) covers private connections, resolved people, character IDs, and authored-asset assignment.

Tom proposed image generation for sketches followed by agent-driven Blender authoring through MCP. This is the preferred workflow for shared assets and the PoC's playable characters. On 2026-09-10 he deferred automatic character generation to [BL-01](../docs/BACKLOG.md#bl-01-automatic-playable-character-generation), to revisit only if he chooses a release beyond the family PoC. No generation worker, mock-job implementation, provider selection, or automation trial blocks the PoC. **Finish documenting technical and nontechnical requirements first; Tom will arrange tool setup afterward.** The concrete Blender host/bridge remains unselected.
