...
## Appendix: bounded ephemeral-run retention

The M2 capacity follow-up applies only to `InMemoryQuestStore.ephemeral()`. Each owner retains its two newest runs; starting another removes the oldest run and its associated preview. At the global save limit the store evicts the least-recently-used run, with successful owned reads and gameplay-action attempts refreshing recency. Retained save records keep their ownership boundary and action receipts. At the session limit the least-recently-used session is evicted and records orphaned by that removal are reclaimed. Preview pressure removes the oldest unreferenced preview and never removes the preview behind a retained save, preserving idempotent `createSave` behavior.

Ephemeral limits must be positive safe integers, and `maxPreviews` must exceed `maxSaves`; the latter guarantees one safe unreferenced preview slot when every save slot is occupied. Normal `InMemoryQuestStore` construction and Postgres behavior are unchanged. Tiny-capacity store tests cover repeated runs for one owner, different-owner global LRU eviction, reads and actions as independent recency touches, receipt replay, owner isolation, session LRU cleanup, expired-session cleanup, unreferenced-preview ordering, retained-preview idempotency, invalid limits and 404 results for every evicted save or preview. The real ephemeral HTTP test starts three runs for one owner at a two-save cap, confirms the superseded ID is gone, then admits another owner and confirms global eviction without `STORE_CAPACITY`.

Focused verification after the final changes: `pnpm typecheck`; ESLint for `memory-store.ts`, `maintenance.test.ts` and `app.test.ts`; 22/22 tests across the maintenance and app files. The coordinator owns the final full-suite run after the remaining PLAN007 integration settles.

## Appendix: held movement through local recovery

The route-memory runtime now distinguishes a local obby recovery from a pause or browser lifecycle reset. Recovery discards queued and held jump, interaction, primary and secondary action buttons while preserving the physically held analog stick and movement keys. A focused input-state test covers simultaneous analog and keyboard movement plus every queued action. Existing full reset paths remain explicit: blur, visibility loss, pause, `clearInput()` and input-binding disposal still clear movement and actions.

The real v3 runtime regression sets forward movement once, deliberately walks off the first gap, observes one recovery at the start checkpoint with the input snapshot still held, then advances another frame and verifies the traveler moves forward from the respawn without another `setInput` call. It also confirms that no gameplay action was dispatched during the fall. Focused verification: `pnpm typecheck`; ESLint for `input.test.ts` and `runtime-obby.test.ts`; 41/41 tests across those two files.
