...
## Final fault test and persistence correction

App PR44's first head `efa2fc2` passed CI, including all 549 tests with 12 real PostgreSQL cases, strict documentation and the container build. The subsequent actual-control run `plan011-retry-live-boss-final` correctly exposed a remaining production defect, so this green head was not merged. Both deliberately stalled automatic recovery requests timed out after eight seconds with the same action ID/revision/action; the manual Return to checkpoint restored the same journey at revision 32→33 with progress intact. The later real in-range Besties Attack, with the fourth ordinary enemy deliberately left alive, returned `503 SAVE_DATA_INVALID`.

The persisted-state validator still enforced archived all-enemies-defeated rules for boss damage, memory release and completed chapters despite the new v2 availability contract. The server lane owns a scoped shared-policy correction and action-service/store boundary regressions through both chapter transitions. The browser lane will repeat the fault test and carry the ordinary-left Besties fight through final memory collection; the failing report/screens remain on the PVC. Runtime-only tests had missed this persistence boundary, and the failure is not attributed to the browser driver. No new release is claimed until those checks and hosted verification pass.
