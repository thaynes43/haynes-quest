...
Lead addendum, now separately assigned to native Sol WO-021: `attach()` sets `finished` before `ready()`, swallowing required-clip/hand-bone callback failure as a finished attachment. The independent agent owns transactional clone/callback recovery and a new test file; WO-019 retains only bitmap cleanup in SceneAssets. Root combines the non-overlapping source diffs. Do not expand this Fable task into the separately owned callback fix.
