...
- **F7:** map cap/expiry fixed in `7a6713b`; global new-session quota and bounded expiry cleanup integrated in `e5bb6d4` (Sol `e1ab8f8`). Every save and referenced/active preview is preserved. Player rows and saves have no automatic retention deletion; that policy remains separate from expired transient records. Fresh PG CI runs the preservation regression.
...
- **F10:** `e5bb6d4` adds safe structured API/startup/maintenance/shutdown diagnostics. Tests assert sentinel private error text, names, cookies and URL parameters never enter logs; only fixed event, built-in error class, method and static route pattern are emitted.
