...
## Final caller integration review

The follow-up read-only audit found that a touch `pointerdown` could create a pending resume before Safari received an activating release. The later `pointerup`/`touchend` then reused that premature promise. Root now ignores touch/pen pointerdown for both normal unlock and celebration, retaining mouse pointerdown and touch release/click/key fallbacks. Caller regressions cover both touch and pen. The UI says **Sound test started** after scheduling; it does not claim a listener heard it.

Primary sources: [HTML activation events](https://html.spec.whatwg.org/multipage/interaction.html#activation-triggering-input-event), [Web Audio pending resume promises](https://webaudio.github.io/web-audio-api/#dom-audiocontext-resume), and [WebKit touchend guidance](https://bugs.webkit.org/show_bug.cgi?id=149367). This corrects a standards mismatch and plausible first-tap failure; exact physical iPhone causality remains unproven.
