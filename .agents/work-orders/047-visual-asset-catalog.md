...
- Keep exact-version evidence and existing deep links. Clearly distinguish completed candidates, concept-only candidates, paused partial models and friendly character candidates. No owner approval is inferred. The Besties and FNAF discussions do not yet have assets.
...
- All 23 source concept/reference images are represented. Review records retain all six friendly-character models and their provenance. PLAN006 and DESIGN013 now define their healing gifts and recoverable harm penalties.
...
## Catalog implementation and browser verification

Root implemented 33 static image cards across seven categories, using 50 WebP derivatives totaling 942,748 bytes. Original model/concept/audio files are unchanged. Each of the 15 exact models in the playtest artwork manifest is labeled “In playtest”; paused Nap/Diva and the six preserved creature studies retain distinct states. Legacy catalog section anchors and all versioned review paths are retained. The original catalog inventory is archived at `docs/assets/catalog-history.md`.

Native Sol’s completed browser audit on September 11 passed desktop1440×1000 and phone390×844: all33 cards and57 image elements, no page overflow, no GLB/WAV requests from the landing page, keyboard and touch card navigation without JavaScript, and the phone Sound auditions category jump. All26 review pages,15 fragments and current inspiration pairings passed; historical keepsakev001 remains reachable while correctedv002 is displayed. All24 delivered GLBs matched MIME, length and exact SHA256. Nap’s existing partial model actually loaded with five expected clips, a visible WebGL canvas and changed touch orbit. No model completion or creative approval is claimed.
