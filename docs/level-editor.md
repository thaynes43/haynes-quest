...
The family journey templates ([DESIGN-024](designs/024-family-journeys.md)) are generated worlds. Each has one generator that owns the whole world, and one pure generator per chapter under `scripts/levels/family/`. World B, `family-world-b@v1` ("Playroom to Big Stage"), is built like this:

- `scripts/levels/family/b1.ts`, `b2.ts` and `b3.ts` each return one chapter's complete authored-level-v4 document (`familyB1Level()`, `buildB2Level()`, `familyB3Level()`), together with the chapter's copy and cast data. Each has its own test under `tests/levels/` with the lints and kid-model traversal evidence.
- `scripts/levels/build-family-world-b.ts` starts from `createWorldEditorProject({ projectId: "family-world-b", catalogVersion: "parody-catalog-v8" })`. `worldShellCommands` sets the admin-facing name, the fictional birth date 2020-06-01, and each chapter's copy, theme, dates, ages and fictional preview memories. The chapters are contiguous: ages 0→2, 2→4 and 4→6. `chapter.level.replace` then places each chapter's level. Finally the generator assigns the casts: catalog references for landed models (Big Honk Bus and the Besties) and project candidates for the rest. Each chapter has one ordinary identity, which fills all four ordinary slots.
- `tsx scripts/levels/build-family-world-b.ts --write` rewrites the command history `scripts/levels/family-world-b.commands.json` and the template `src/shared/levels/family-world-b-v1.json`. `pnpm levels:validate` replays every `src/shared/levels/family-world-<key>-v<N>.json` from its command file byte-identically: `family-world-<key>.commands.json` for v1 and `family-world-<key>-v<N>.commands.json` after that.
- `CHECKED_IN_FAMILY_TEMPLATES` in `src/server/family/templates.ts` registers the template by id and version. The project's name is what administrators see when they pick a template. A published journey freezes the template's fingerprint, so a registered version is never edited. A change becomes a new version, with its own JSON and command file, registered beside the old one.

A template is offered only when its rebased world validates for the child: the final chapter's big memory must already be due, and every cast window must hold at each chapter's rebased start. For World B, that means a child who is at least six and born in 2020 or later: the Demon Idol and Besties windows open on 2024-01-01, and chapter three starts on the fourth birthday.
...
`tests/e2e/family-world.ts` plays each chapter in Chromium through the ephemeral editor playtest, with the page clock in lockstep. Serve a fresh build in the ephemeral fixture mode described in the [private-preview runbook](ops/002-private-preview.md), then run `QUEST_E2E_URL=http://127.0.0.1:3000 node_modules/.bin/tsx tests/e2e/family-world.ts`. It drives the game only through its analog input. Before each crossing it plans the moves with the chapter tests' patient-kid planner, starting from the game's own traversal state and course clock, then replays the planned stick frame by frame. It collects both tools, fights every encounter, recovers both minor memories and the major memory, and saves the spawn, mid-climb and boss-arena screenshots with a `report.json`. This proves route logic and rules under ordinary input, not frame time or feel on a device.
