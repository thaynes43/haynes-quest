...
- **Integration:** The exact v001 model is placed as scenery in two private family world templates (PLAN-019): Clubhouse to Casino (`family-world-a@v1` and `@v2`, same decor), 27 placements in The Toon Clubhouse and 5 in Harbor Rescue; and Playroom to Big Stage (`family-world-b@v1`), 23 placements in The Magic House. It is the shared theme-kit prop `clearing-tree` (DESIGN-025 D-05). Decor has no collision, and a model that fails to load draws its procedural fallback. This records use, not approval. The other private playtest courses use temporary shapes. Changed geometry or materials require a new candidate version before approval or use.
