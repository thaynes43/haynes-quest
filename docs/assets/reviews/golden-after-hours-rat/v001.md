...
The browser check used software Chromium. Physical iPad/iPhone Safari, device frame time and combat feel remain untested. This exact `v001` GLB was pinned as the noncombat stage cameo on the private fictional `rat-casino-v1` route after Tom's September 23 “Looks great, let’s move forward” direction. Tom's September 25 playtest found that character unresponsive. The new `rat-casino-v2` route uses the same exact model as an **optional ordinary fight** in the right stage wing: it can be targeted, attack and be defeated using the existing server-owned combat rules. Rat and the final memory remain available if Golden is skipped. The five authored clips and GLB bytes are unchanged; the client no longer spawns a second scenic copy in that route. Its SHA-256 is `b0ac1fd11f4df8039e69a9b303b1eb3e8342654b795b08db8fd8f68559b22184`. Final art acceptance follows the live level and device review.
