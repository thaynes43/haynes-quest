...
**Audio candidate · in the private playtest · final listening review pending.** A quiet confirmation for an interface choice. The prompt asks for a rounded wooden tick with a tiny warm ping.
...
[DESIGN-022](../../../designs/022-rat-casino-fun-pass.md) keeps this WAV unchanged and adds a use. In Rat Casino, each casino token plays it at 1.25× speed and 0.8 of its level. Tokens taken within 0.7 s of each other form a streak, and each plays a little faster, up to the 1.5× limit. A landed attack now plays the existing impact variant when the swing makes contact; the server's confirmation no longer repeats it. These uses still need listening on the physical iPad and iPhone.
