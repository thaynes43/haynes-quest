...
**Three completed environment model candidates · Ready for Tom · Cabinet and bollard in the private family world as scenery.** The coordinator selected the source concept and compared the exported kit with it. Tom has not reviewed or approved these exact models for gameplay.
...
The exact three-model assembly carries the concept's crescent silhouette and honey light. The render uses simpler, flatter faces than the illustration. The correct side render includes both arch feet. The models are decorative, with no collision or interaction. The exact v001 cabinet (2 placements) and joystick bollard (9) are placed as scenery in Hero City, chapter 3 of the private family world template Clubhouse to Casino (`family-world-a@v1` and `@v2`, same decor, PLAN-019); the level validator keeps them out of walkable space, connection lanes and fight areas. The ticket arch is not placed, because its bounding box includes the opening and cannot straddle a lane.
...
- **Use:** Cabinet and bollard: private family world scenery (`family-world-a@v1` and `@v2` decor) since PLAN-019; this records use, not approval. Ticket arch: asset studio only. Other private playtest courses still use procedural arcade scenery. Changed geometry or materials need a new candidate version before approval or integration.
