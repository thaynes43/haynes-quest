...
**Three completed environment model candidates · Ready for Tom · The wind-up lantern is scenery in the private family World B; the tower and rail are studio only.** The coordinator selected the source concept and inspected the exact exported assembly. Tom's versioned review remains open.
...
**Gameplay use (private candidate).** The exact v001 lantern is registered as the shared theme-kit prop `toybox-windup-lantern` (DESIGN-025 D-05). The Magic House, chapter 2 of the private family template `family-world-b@v1`, places it 27 times as floating candle scenery beside the lanes. Decor has no collision. If the model fails to load, its procedural fallback draws instead. This records use in the private family release, not Tom's approval.
...
The block tower and rail use simpler painted surfaces than the illustrated world. The lantern's key and glow were corrected during technical intake. The tower and rail have no course placement and have not entered the game manifest. The lantern is placed only as decor, with no collision, in `family-world-b@v1`.
...
- **Use:** The tower and rail are asset studio only. The wind-up lantern is private-candidate scenery in the family template `family-world-b@v1` (The Magic House). The fictional playtest still uses procedural toybox scenery. Changed geometry or materials need a new candidate version before approval or integration.
