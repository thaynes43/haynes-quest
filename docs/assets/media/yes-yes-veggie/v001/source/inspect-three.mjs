/** WO111 yes-yes-veggie: exact GLTFLoader, all-vertex bounds and the actual game adapter. */
...
const root=path.resolve(process.argv[2]),bytes=await readFile(path.join(root,'yes-yes-veggie.glb'));
...
// Leaf-mitten tip in rest (Blender wrist + 0.182 m along the leaf axis → glTF (x, z, -y)), carried in each hand bone frame.
const tipLocal={};
for(const s of ['L','R']){
 const lf=construction.leaves['hand_'+s],w=lf.wrist,a=lf.w_axis,b=[w[0]+.182*a[0],w[1]+.182*a[1],w[2]+.182*a[2]];
 tipLocal[s]=new T.Vector3(b[0],b[2],-b[1]).applyMatrix4(scene.getObjectByName('hand_'+s).matrixWorld.clone().invert());
}
const tipWorld=(s,sc=scene)=>tipLocal[s].clone().applyMatrix4(sc.getObjectByName('hand_'+s).matrixWorld);
const bodyWorld=(sc=scene)=>sc.getObjectByName('body').getWorldPosition(new T.Vector3());
function points(){
 scene.updateMatrixWorld(true);
...
const restTips={L:tipWorld('L'),R:tipWorld('R')},restBody=bodyWorld();
const restBonePositions=Object.fromEntries(bones.map(b=>[b.name,b.position.clone()]));
// Rotation-only joints keep their local pivot. Hips bob/lunge, feet lift about a sole pivot, pupils glance and brows lift by design.
const attachments=Object.fromEntries(bones.filter(b=>!['root','hips','leg_L','leg_R','pupils','brow_L','brow_R'].includes(b.name)).map(b=>[b.name,0]));
...
for(const clip of gltf.animations){
 ...
 const total=Math.ceil(clip.duration*60),bounds=new T.Box3();let first,last,hold,heldDifference=0,maxDisplacement=0,floorCause={y:Infinity},minTipY=Infinity;
 for(let i=0;i<=total;i++){
  ...
  if(clip.name==='defeat'&&time>=1.70){if(!hold)hold=sample.positions;else for(let j=0;j<hold.length;j++)heldDifference=Math.max(heldDifference,hold[j].distanceTo(sample.positions[j]));}
  minTipY=Math.min(minTipY,tipWorld('L').y,tipWorld('R').y);
...
const contact=[];
for(const t of [0,.30,.60,.72,.84,1.05,1.15,1.25,1.32,1.60,2]){
 mixer.setTime(t);const sample=points();
 contact.push({time_s:t,body:bodyWorld().toArray(),leaf_tip_L:tipWorld('L').toArray(),leaf_tip_R:tipWorld('R').toArray(),hand_L:scene.getObjectByName('hand_L').getWorldPosition(new T.Vector3()).toArray(),bounds_y_up:pack(sample.box)});
}
...
const firstClone=clone(scene),secondClone=clone(scene),frozenSource=bones.map(b=>b.matrix.toArray()),hand2=secondClone.getObjectByName('hand_L').quaternion.clone();
const adapter=new EnemyAnimation(firstClone,gltf.animations,.625);
const frame={id:'wo111-veggie-inspection',position:{x:0,y:0,z:0},facing:0,phase:'idle',windupProgress:0,hp:100,maxHp:100};
const phases=[];
for(const phase of ['idle','chasing','windup','strike','cooldown']){
 ...
 firstClone.updateMatrixWorld(true);phases.push({phase,visible:firstClone.visible,body:bodyWorld(firstClone).toArray(),hand_L:firstClone.getObjectByName('hand_L').getWorldPosition(new T.Vector3()).toArray()});
}
...
const cloneIndependent=bones.every((b,i)=>b.matrix.toArray().every((x,j)=>Math.abs(x-frozenSource[i][j])<1e-10))&&secondClone.getObjectByName('hand_L').quaternion.toArray().every((x,i)=>Math.abs(x-hand2.toArray()[i])<1e-10);
const storedBounds=scene.getObjectByName('Yes_Yes_Veggie_Skin').userData.model_space_bounds_y_up;
...
const at=s=>contact.find(c=>c.time_s===s),V=a=>new T.Vector3(...a);
const warning=at(.84),strike=at(1.25);
const heldWarning=contact.filter(s=>s.time_s>=.60&&s.time_s<=1.05).every(s=>['leaf_tip_L','leaf_tip_R','body'].every(k=>V(s[k]).distanceTo(V(warning[k]))<.00001));
const feet=[...rest.groups.leg_L,...rest.groups.leg_R].filter(p=>p.y<.02),footprint=new T.Box3().setFromPoints(feet);
const atWindup=phases.find(p=>p.phase==='windup');
const vertexColors=meshes.every(m=>m.geometry.attributes.color&&m.material.vertexColors===true);
const checks={exported_bounds_cover_all_poses:!!storedBounds&&union.min.toArray().every((v,i)=>v>=storedBounds.min[i])&&union.max.toArray().every((v,i)=>v<=storedBounds.max[i]),
 ...
 height_0_8_to_1_1_m:rest.box.max.y>=.8&&rest.box.max.y<=1.1&&Math.abs(rest.box.max.y-construction.height_m)<.001,feet_on_floor_at_rest:Math.abs(rest.box.min.y)<.0005,floor_centred_footprint:Math.abs(footprint.max.x+footprint.min.x)<.002&&Math.abs((footprint.max.z+footprint.min.z)/2)<.03,
 one_1024_embedded_atlas:images.length===1&&images[0].width===1024&&images[0].height===1024,two_draw_primitives:meshes.length===2,baked_occlusion_vertex_colors:vertexColors,actual_adapter_constructed:true,actual_adapter_defeat_vanish:state.visible===false&&state.vanish===1,adapter_root_identity:firstClone.position.length()<1e-8&&firstClone.scale.distanceTo(new T.Vector3(1,1,1))<1e-8,independent_skeleton_clone:cloneIndependent,
 attack_contact_1_25:Math.abs(animations.attack.duration_s*.625-1.25)<1e-9,actual_adapter_contact_pose:V(atWindup.hand_L).distanceTo(directContactHand)<1e-5,warning_held_0_60_to_1_05:heldWarning,
 warning_leaves_raised:['leaf_tip_L','leaf_tip_R'].every(k=>V(warning[k]).y>V(at(0)[k]).y+.25),warning_crouch:V(warning.body).y<restBody.y-.03,
 shove_body_forward:V(strike.body).z<restBody.z-.06,shove_leaves_forward:['leaf_tip_L','leaf_tip_R'].every(k=>V(strike[k]).z<V(warning[k]).z-.08),
 leaf_tips_above_floor:Object.values(animations).every(a=>a.minimum_leaf_tip_y_m>.01)};
adapter.dispose();
...
const record={asset_id:'yes-yes-veggie',version:'v001',glb_sha256:createHash('sha256').update(bytes).digest('hex'),three_revision:T.REVISION,method:'Exact GLTFLoader; all exported skin vertices at 60 Hz; SkeletonUtils clone; actual src/game/enemy-animation.ts. Rotation-only joints are checked as fixed local pivots; the hips bob and lunge, the feet lift about a sole pivot, and the pupils and brows move by design. Feet rest on the floor. No physical-device or universal triangle-collision proof.',rest_bounds_y_up:pack(rest.box),all_animation_bounds_y_up:pack(union),dimensions_m:rest.box.getSize(new T.Vector3()).toArray(),footprint_y_up:pack(footprint),exported_safe_bounds_y_up:storedBounds,animations,images,root_bone_tracks:rootBoneTracks,tracks:bindings,attachment_pivot_max_drift_m:attachments,max_skin_influences:maxInfluences,max_weight_sum_error:maxWeightError,contact,adapter:{phases,direct_contact_hand_L:directContactHand.toArray(),hit_hand_L:hitHand,defeat_states:defeatStates},checks};
await writeFile(path.join(root,'three-inspection.json'),JSON.stringify(record,null,2)+'\n');
const failures=Object.entries(checks).filter(([,v])=>!v).map(([k])=>k);console.log(JSON.stringify({failures,rest:record.rest_bounds_y_up,animated:record.all_animation_bounds_y_up,floor:Object.fromEntries(Object.entries(animations).map(([k,a])=>[k,[+a.bounds_y_up.min[1].toFixed(5),a.floor_minimum.bone,+a.minimum_leaf_tip_y_m.toFixed(3)]])),attachments,maxInfluences,footprint:record.footprint_y_up,contact:contact.map(c=>[c.time_s,c.body.map(v=>+v.toFixed(3)),c.leaf_tip_L.map(v=>+v.toFixed(3))])}));process.exitCode=failures.length?1:0;
