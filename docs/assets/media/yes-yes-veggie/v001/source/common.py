"""WO111 yes-yes-veggie mesh, atlas UV and skin helpers.

Adapted from the WO111 gadget-helper and honk-bus helpers: the same
rigid-part plus blended-weight construction and superellipse lofts, with
added custom tile UV mapping (mirrored cylindrical stalk, planar leaf and
radial cheek gradients) and per-point tapered sweeps for brows.
...
BASE=Path('/workspace/haynes-quest/family-eras/yes-yes-veggie')
...
def setup(name,materials):
 ...
 image=bpy.data.images.load(str(CURRENT/'pigment.png'),check_existing=False);image.name='yes-yes-veggie original painted 1024 atlas';image.pack()
...
def tile_uv(tile,u,v):
 ...
 return ((tile%4+TILE_INSET+TILE_SPAN*u)/4,(3-tile//4+TILE_INSET+TILE_SPAN*v)/4)

def custom_uv(ob,fn):
 """Replace a part's UVs: fn(vertex_co, polygon) -> (tile, u, v) in 0..1 tile space."""
 uv=ob.data.uv_layers.active
 for p in ob.data.polygons:
  for li in p.loop_indices:
   co=ob.data.vertices[ob.data.loops[li].vertex_index].co
   t,u,v=fn(co,p);uv.data[li].uv=tile_uv(t,u,v)

def taper_sweep(name,points,half_u,half_v,e,tile,bn='body',mat=0,n=10,weights=None,hint=(0,1,0),caps=True):
 """Rounded-rectangle ribbon along a path with per-point half sizes (brows)."""
 pts=[Vector(p) for p in points];rr=[]
 for i,(p,(t,u,v)) in enumerate(zip(pts,frames(pts,hint))):
  hu=half_u[i] if isinstance(half_u,(list,tuple)) else half_u;hv=half_v[i] if isinstance(half_v,(list,tuple)) else half_v
  rr.append([p+x*u+y*v for x,y in (superellipse(hu,hv,e,math.tau*j/n) for j in range(n))])
 return rings(name,rr,tile,bn,mat,caps,True,weights)
