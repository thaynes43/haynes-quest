"""WO111 yes-yes-veggie saved exclusive scene release; final GLB remains untouched."""
import bpy,json,hashlib,datetime
from pathlib import Path
ROOT=Path('/workspace/haynes-quest/family-eras/yes-yes-veggie/v001')
sc=bpy.context.scene
assert sc.get('work_order')=='WO111' and sc.get('asset_id')=='yes-yes-veggie' and sc.get('scene_lease')=='active'
...
glb_hash=sha(ROOT/'yes-yes-veggie.glb')
...
record={'work_order':'WO111','asset_id':'yes-yes-veggie','version':'v001','released_utc':now,'scene_owner':'none','scene_lease':'released','saved_live_scene':str(ROOT/'live-scene-release.blend'),'saved_live_scene_sha256':sha(ROOT/'live-scene-release.blend'),'editable_master':str(ROOT/'yes-yes-veggie.blend'),'editable_master_sha256':sha(ROOT/'yes-yes-veggie.blend'),'glb_sha256':glb_hash,'author_jobs_running':any(jobs.values()),'job_checks':jobs,'prior_masters_unchanged':lease['prior_masters'],'takeover':lease.get('takeover'),'scope':"Awaiting Tom's review · used in the family release. Candidate technical delivery only; lead owns publication and runtime integration. No physical-device acceptance."}
(ROOT/'scene-release.json').write_text(json.dumps(record,indent=2)+'\n');print(json.dumps({k:record[k] for k in ['released_utc','scene_lease','glb_sha256','saved_live_scene_sha256','editable_master_sha256']}))
