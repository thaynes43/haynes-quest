...
const c=new T.OrthographicCamera(-1.05,1.05,1.18,-1.18,.01,30);const g=await new GLTFLoader().loadAsync('/asset.glb');s.add(g.scene);const m=new T.AnimationMixer(g.scene);
window.draw=(view='beauty',clip=null,fraction=0,scale=1)=>{m.stopAllAction();if(clip){const a=m.clipAction(g.animations.find(a=>a.name===clip));a.setLoop(T.LoopOnce,1);a.clampWhenFinished=true;a.play();m.setTime(a.getClip().duration*fraction)}g.scene.updateMatrixWorld(true);const v={front:[0,.58,-6],side:[6,.58,0],back:[0,.58,6],beauty:[3.1,1.52,-6],detail:[-4,1.2,-5],hinge:[4,1.2,5]}[view];c.position.set(...v);c.lookAt(0,view==='detail'||view==='hinge'?.8:.58,0);c.zoom=scale*(view==='detail'||view==='hinge'?2.25:1);c.updateProjectionMatrix();r.render(s,c);return {clips:g.animations.map(a=>({name:a.name,duration:a.duration})),calls:r.info.render.calls,triangles:r.info.render.triangles,glError:r.getContext().getError(),webglVersion:r.getContext().getParameter(r.getContext().VERSION),pose:g.scene.getObjectByName('wrench').getWorldPosition(new T.Vector3()).toArray()}};
window.playClip=async(clip,scale=1)=>{window.draw('beauty',clip,0,scale);const duration=g.animations.find(a=>a.name===clip).duration,start=performance.now(),poses=[];let previous=start;while((performance.now()-start)/1000<duration){await new Promise(requestAnimationFrame);const now=performance.now();m.update(Math.min(.1,(now-previous)/1000));previous=now;g.scene.updateMatrixWorld(true);r.render(s,c);poses.push({time_s:(now-start)/1000,hand:g.scene.getObjectByName('wrench').getWorldPosition(new T.Vector3()).toArray()})}return {elapsed_s:(performance.now()-start)/1000,frames:poses.length,poses,glError:r.getContext().getError(),scale}};window.ready=true;window.draw();</script>`;
...
try{
 ...
 if(process.argv.includes('--early')||process.argv.includes('--intake')){
 if(process.argv.includes('--intake'))for(const view of ['front','side','beauty']){await page.evaluate(view=>window.draw(view,'defeat',1),view);await page.screenshot({path:path.join(out,'defeat-'+view+'-held.png')});}
 ...
 for(const view of ['front','side','beauty']){await page.evaluate(view=>window.draw(view,'defeat',1),view);await page.screenshot({path:path.join(out,'defeat-'+view+'-held.png')});}
