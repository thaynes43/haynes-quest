"""WO111 magic-house: supported MCP/HTTP transfer between Blender instance 2 and the durable artifact directory.
...
The sheet-stage files (reference sheet, blockout, sheet scripts, previews, the sheet's own lease/release/manifest)
...
from transfer import MCP, REMOTE, ARTIFACTS, upload as _upload
LOCAL = Path('/home/dev/artifacts/haynes-quest/family-eras/magic-house/v001')
SOURCE = Path(__file__).resolve().parent
sha = lambda data: hashlib.sha256(data).hexdigest()
def upload(client, local, relative): return _upload(client, Path(local).read_bytes(), REMOTE + '/' + relative)
MASTERS = ['magic-house.blend', 'magic-house.glb', 'pigment.png', 'construction.json', 'validation.json', 'attachment-inspection.json', 'magic-house-construction.blend', 'magic-house-checkpoint.glb', 'claim-checkpoint.blend']
...
REPORTS = ['three-inspection.json', 'visual-review.json', 'provenance.json', 'tool-settings.json', 'bounds.json', 'delivery-record.json', 'sheet-vs-export.png']
INPUTS = ['reference-sheet.png', 'reference-notes.md', 'magic-house-blockout.blend', 'blockout-measurements.json']
...
SOURCE_FILES = ['transfer.py', 'claim.py', 'common.py', 'build.py', 'animate.py', 'inspect-attachments.py', 'validate.mjs', 'inspect-three.mjs', 'preview.mjs', 'look.mjs',
                'grid.mjs', 'compare.mjs', 'make-evidence.py', 'delivery.py', 'release_model.py', 'cycle.sh', 'bounds.json']
...
def preserve_sheet_records():
    ...
    (dest / 'README.md').write_text('Records of the reference-sheet stage (blender-authoring instance 1, Sept 26 02:30-02:58 UTC), kept when the\n'
...
def check_reports():
    expected = sha((LOCAL / 'magic-house.glb').read_bytes())
...
if mode == 'fetch':
    ...
    print(json.dumps({'fetched': MASTERS, 'glb_sha256': sha((LOCAL / 'magic-house.glb').read_bytes())}))
elif mode == 'stage':
    client = MCP()
    client.execute("import bpy; sc=bpy.context.scene; assert sc.get('work_order')=='WO111' and sc.get('asset_id')=='magic-house' and sc.get('scene_lease')=='active'")
    for name in SOURCE_FILES:
        shutil.copy2(SOURCE / name, LOCAL / 'source' / name); upload(client, SOURCE / name, 'source/' + name)
    ...
    mirrored = MASTERS + STILLS + REPORTS + INPUTS + ['scene-lease.json', 'scene-release.json', 'live-scene-release.blend', 'claim-checkpoint.blend', 'source/rig-rest.json', 'source/build_blockout.py']
    mirrored += ['source/' + n for n in SOURCE_FILES]
    mirrored += ['final-preview/' + p.name for p in sorted((LOCAL / 'final-preview').iterdir()) if p.suffix in ('.png', '.json')]
    local_only = ['sheet-stage/' + p.name for p in sorted((LOCAL / 'sheet-stage').iterdir())] + ['preview/' + p.name for p in sorted((LOCAL / 'preview').iterdir())]
    local_only += ['magic-house-reference-sheet.blend'] + ['source/' + n for n in ('crops.py', 'release.py', 'run.py', 'sheet.py', 'sheet_preview.py')]
    files = []; seen = set()
    for name in mirrored:
        ...
        else: assert sha(data) == sha(urlopen(ARTIFACTS + name, timeout=120).read()), name
    ...
    record = {'work_order': 'WO111', 'asset_id': 'magic-house', 'version': 'v001', 'status': 'model-delivered', 'source': 'Blender reference sheet, no generated concept',
              ...
              'excluded': ['sheet-stage/ holds the reference-sheet stage lease, release and manifest (instance 1); they are listed but not mirrored on instance 2.',
                           'work/ holds author iteration renders and superseded GLBs; not part of the delivery and not listed.'],
    ...
    print(json.dumps({'stage': 'complete', 'files': len(files), 'manifest_sha256': sha((LOCAL / 'sha256-manifest.json').read_bytes()), 'scene_lease': 'released', 'glb_sha256': expected,
                      'master_sha256': sha((LOCAL / 'magic-house.blend').read_bytes())}))
