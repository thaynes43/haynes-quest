...
const root=path.resolve(process.argv[2]),bytes=await readFile(path.join(root,'honk-bus.glb'));
...
const bodyRest=scene.getObjectByName('body').matrixWorld.clone();
const roofRestY=(1.776-construction.construction_ground_shift)*construction.construction_scale_factor;
const tipRest=new T.Vector3(0,(2.123+.145*.60-construction.construction_ground_shift)*construction.construction_scale_factor,-(.60-construction.construction_forward_center)*construction.construction_scale_factor);
const hornTipLocal=tipRest.clone().applyMatrix4(scene.getObjectByName('horn').matrixWorld.clone().invert());
function points(){
 scene.updateMatrixWorld(true);
 ...
 const bodyUnpose=bodyRest.clone().multiply(scene.getObjectByName('body').matrixWorld.clone().invert());
 const hornRoofClearance=Math.min(...groups.horn.map(p=>p.clone().applyMatrix4(bodyUnpose).y-roofRestY));
 return {positions,groups,minimum,box:new T.Box3().setFromPoints(positions),hornRoofClearance};
}
...
const attachments=Object.fromEntries(bones.filter(b=>!['root','body','horn_lift','lid_-1','lid_1'].includes(b.name)).map(b=>[b.name,0]));
let rootFixed=true,maxInfluences=0,maxWeightError=0,minHornRoofClearance=Infinity;
...
for(const clip of gltf.animations){
 ...
 const total=Math.ceil(clip.duration*60),bounds=new T.Box3();let first,last,hold,heldDifference=0,maxDisplacement=0,floorCause={y:Infinity},hornMin=Infinity;
 for(let i=0;i<=total;i++){
  ...
  hornMin=Math.min(hornMin,sample.hornRoofClearance);
 ...
 animations[clip.name]={duration_s:clip.duration,samples:total+1,all_vertices_per_sample:rest.positions.length,max_temporal_vertex_displacement_m:maxDisplacement,bounds_y_up:pack(bounds),loop_seam_max_m:seam,floor_minimum:floorCause,minimum_horn_roof_clearance_m:hornMin,held_defeat_difference_m:clip.name==='defeat'?heldDifference:null};union.union(bounds);minHornRoofClearance=Math.min(minHornRoofClearance,hornMin);
}
...
const contact=[];
for(const t of [0,.60,.72,.84,1.05,1.25,1.40,2]){
 mixer.setTime(t);const sample=points();contact.push({time_s:t,horn:scene.getObjectByName('horn').getWorldPosition(new T.Vector3()).toArray(),horn_tip:hornTipLocal.clone().applyMatrix4(scene.getObjectByName('horn').matrixWorld).toArray(),horn_quaternion:scene.getObjectByName('horn').getWorldQuaternion(new T.Quaternion()).toArray(),horn_bounds_y_up:pack(new T.Box3().setFromPoints(sample.groups.horn)),body_bounds_y_up:pack(new T.Box3().setFromPoints(sample.groups.body)),bounds_y_up:pack(sample.box)});
}
const directContactHorn=new T.Vector3(...contact.find(s=>s.time_s===1.25).horn);
mixer.stopAllAction();scene.updateMatrixWorld(true);
const firstClone=clone(scene),secondClone=clone(scene),frozenSource=bones.map(b=>b.matrix.toArray()),horn2=secondClone.getObjectByName('horn').quaternion.clone();
const adapter=new EnemyAnimation(firstClone,gltf.animations,.625);
const frame={id:'wo111-bus-inspection',position:{x:0,y:0,z:0},facing:0,phase:'idle',windupProgress:0,hp:100,maxHp:100};
const phases=[];
for(const phase of ['idle','chasing','windup','strike','cooldown']){
 ...
 firstClone.updateMatrixWorld(true);phases.push({phase,visible:firstClone.visible,body:firstClone.getObjectByName('body').getWorldPosition(new T.Vector3()).toArray(),horn:firstClone.getObjectByName('horn').getWorldPosition(new T.Vector3()).toArray()});
}
frame.hp=70;frame.phase='idle';adapter.update(frame,.1);firstClone.updateMatrixWorld(true);const hitHorn=firstClone.getObjectByName('horn').getWorldPosition(new T.Vector3()).toArray();
...
const cloneIndependent=bones.every((b,i)=>b.matrix.toArray().every((x,j)=>Math.abs(x-frozenSource[i][j])<1e-10))&&secondClone.getObjectByName('horn').quaternion.toArray().every((x,i)=>Math.abs(x-horn2.toArray()[i])<1e-10);
const storedBounds=scene.getObjectByName('Honk_Bus_Skin').userData.model_space_bounds_y_up;
...
const heldWarning=contact.filter(s=>s.time_s>=.60&&s.time_s<=1.05).every(s=>new T.Vector3(...s.horn_tip).distanceTo(new T.Vector3(...warning.horn_tip))<.00001);
const checks={exported_bounds_cover_all_poses:!!storedBounds&&union.min.toArray().every((v,i)=>v>=storedBounds.min[i])&&union.max.toArray().every((v,i)=>v<=storedBounds.max[i]),
 ...
 no_floor_penetration:Object.values(animations).every(a=>a.bounds_y_up.min[1]>=-.00001),attached_joint_pivots:Object.values(attachments).every(d=>d<.00001),horn_above_body_roof:minHornRoofClearance>=-.00001,at_most_four_influences:maxInfluences<=4,normalized_weights:maxWeightError<.00001,identity_root_no_motion:rootFixed,no_root_bone_motion:rootBoneTracks.every(t=>t.constant),correct_height:Math.abs(rest.box.max.y-2.3)<.001,floor_centered_rest:Math.abs(rest.box.min.y)<.00001&&Math.abs(rest.box.max.x+rest.box.min.x)<.00001&&Math.abs(rest.box.max.z+rest.box.min.z)<.00001,
 ...
 attack_contact_1_25:Math.abs(animations.attack.duration_s*.625-1.25)<1e-9,actual_adapter_contact_pose:new T.Vector3(...atWindup.horn).distanceTo(directContactHorn)<1e-5,warning_held_0_60_to_1_05:heldWarning,warning_horn_lift:warning.horn_tip[1]>contact[0].horn_tip[1]+.15,forward_honk_bounce:strike.horn_tip[2]<warning.horn_tip[2]-.20};
adapter.dispose();
...
const record={asset_id:'honk-bus',version:'v001',glb_sha256:createHash('sha256').update(bytes).digest('hex'),three_revision:T.REVISION,method:'Exact GLTFLoader; all exported skin vertices at 60 Hz; SkeletonUtils clone; actual src/game/enemy-animation.ts. Joint attachment checked as fixed local pivots except intentional body suspension, vertical horn extension and sliding toy eyelids. Every horn vertex is tested above the unposed body roof height. Body/wheel clearance also receives source-geometry and visual review. No physical-device or universal triangle-collision proof.',rest_bounds_y_up:pack(rest.box),all_animation_bounds_y_up:pack(union),dimensions_m:rest.box.getSize(new T.Vector3()).toArray(),exported_safe_bounds_y_up:storedBounds,animations,images,root_bone_tracks:rootBoneTracks,tracks:bindings,attachment_pivot_max_drift_m:attachments,max_skin_influences:maxInfluences,max_weight_sum_error:maxWeightError,minimum_horn_roof_clearance_m:minHornRoofClearance,contact,adapter:{phases,direct_contact_horn:directContactHorn.toArray(),hit_horn:hitHorn,defeat_states:defeatStates},checks};
