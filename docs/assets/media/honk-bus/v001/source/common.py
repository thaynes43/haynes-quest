"""WO111 reusable mesh, atlas UV and skin helpers, inspected from WO099.
...
BASE=Path('/workspace/haynes-quest/family-eras/honk-bus')
...
def setup(name,height,contact,duration,materials):
 ...
 image=bpy.data.images.load(str(CURRENT/'pigment.png'),check_existing=False);image.name=name+' original painted 1024 atlas';image.pack()
...
def box(name,c,s,tile,bn='body',bevel=.008,segments=3,mat=0,rot=(0,0,0),weights=None):
 ...
 if bevel:
  mod=ob.modifiers.new('Softly tailored toy edge','BEVEL');mod.width=bevel;mod.segments=3 if name=='Continuous honey yellow rounded bus body' else min(segments,2);bpy.ops.object.modifier_apply(modifier=mod.name)
...
def ellipsoid(name,c,s,tile,bn='body',mat=0,n=24,r=12,rot=(0,0,0),weights=None):
 n=max(8,round(n*.38/2)*2);r=max(4,round(r*.38))
