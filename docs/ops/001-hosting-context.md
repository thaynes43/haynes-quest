...
[ADR-002](../adrs/002-web-game-stack.md) proposes one Node/Hono application image serving the browser build and API, plus dedicated Postgres storage for configuration, people, journey definitions, memory progress, sessions, and saves. Deployment design must cover migrations, encrypted API-key storage with separately delivered key material, backup/restore, protected photos/timeline metadata, shared avatar assets, and credential revocation. [DESIGN-002](../designs/002-asset-pipeline.md) and PLAN-003 place developer authoring tools in a separate cluster Blender/MCP workload with its own PVC, internal endpoint, and artifact transfer. This workload can roll independently of dev-env and the game. Optional GPU render Jobs and a separate audio-model service need their own resource decisions; offered hardware is not a deployed dependency. The family PoC loads prepared models; a generation worker is not a rollout requirement. Automatic generation is conditional future [BL-01](../BACKLOG.md). These resources have not been created.
