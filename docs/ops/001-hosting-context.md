...
[ADR-002](../adrs/002-web-game-stack.md) proposes one Node/Hono application image serving the browser build and API, plus dedicated Postgres storage for configuration, characters, asset assignments, sessions, and saves. Deployment design must cover migrations, encrypted API-key storage with separately delivered key material, backup/restore, private authored assets, and credential revocation. [DESIGN-002](../designs/002-asset-pipeline.md) describes the developer authoring tools whose setup follows requirements documentation. The family PoC loads prepared models; a generation worker is not a rollout requirement. Automatic generation is conditional future [BL-01](../BACKLOG.md). These resources have not been created.
