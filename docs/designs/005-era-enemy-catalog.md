...
- **Satisfies:** [PRD-001 R-20–R-29](../prds/001-project-brief.md)
- **Related:** [Memory journey](004-memory-journey.md), [saved games](001-technical-foundation.md), [asset authoring](002-asset-pipeline.md)
...
| Known birth information and available photo coverage | Age labels and the grouping/pacing choices in DESIGN-004. Age alone does not identify a cultural era. |
...
| Player challenge settings and chapter progression | Enemy challenge and encounter complexity under the eventual gameplay rules, independently of subject age or gender. |
...
| D-04 | Encounter role and behavior | Identify ordinary-enemy and boss roles, gameplay behaviors, animation requirements, and difficulty parameters separately from visual/theme tags. Both enemies and bosses must satisfy the same era constraints. Boss counts, placement, and victory conditions are undecided. |
...
| D-06 | Selection order | Select eligible entries by represented calendar period first, then apply the agreed preference and variety rules. A later-era entry never becomes a fallback for an earlier period merely because it matches a preference. |
...
| D-08 | Missing catalog coverage | Preserve the photo timeline. Use an explicitly authored, period-neutral fallback suitable for ordinary/boss roles, or show an actionable content-availability state before play. Do not skip someone's memories, fabricate a historical reference, or silently select an incompatible era. The fallback roster and UI remain to be designed. |
...
- Two subjects pictured at the same age in different decades receive different era-eligible pools. The same historical period remains consistent even when the subjects' ages differ.
