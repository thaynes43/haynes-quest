...
The private editor pins `parody-catalog-v6` for the `rat-casino-v1` period without changing archived v1–v5 plans or the default v5 project. Its curated relevance window is January 2024 through December 2026. Chick-flia, Jackrabbit Drummer, Fox Card Shark and Moth Projectionist fill ordinary-a/b/a/b; Rat Pit Boss uses its exact `v002` asset in the boss slot. Golden After-Hours Rat has a frozen catalog identity. It is excluded from the five required selections and was an idle scenic cameo on `rat-casino-v1`; [DESIGN-023](023-golden-optional-encounter-and-mouse.md) admits it to the separate optional ordinary slot on `rat-casino-v2`. The period parser rejects a start before August 18, 2014, and date eligibility rejects chapter starts outside the curated window. The project uses 2025–2026 fictional dates. The models' five authored clips use the existing telegraphed behavior presets; no wagering system or surprise jump scare is inferred from their names.
