...
World A is assembled as the family template `family-world-a@v2`, "Clubhouse to Casino", by `scripts/levels/build-family-world-a.ts` ([level editor: family worlds](../level-editor.md#family-worlds)). v1 stays registered for journeys already published on it. v2 makes three fixes:

- it removes The Toon Clubhouse's mop sweeper from the boss pad's run-up;
- it widens the casino's optional On-Air pad;
- it gives all six lifts deep cars, so a child who walks at an empty shaft no longer falls in.

The chapters climb to 14.8, 13.4, 15.0 and 22.3 m, and the casino finale has the tallest climb and the longest traversal. Lifts and bounce pads carry 12.4 m of the opener's climb. The climb earned by jumping grows from 3.3 m to 6.8 m to 9.3 m over the first three chapters, and the required move gates grow from none to 8 to 8 to 10 in the finale. The web-slinging helper is future vocabulary: no friendly assist mechanic exists yet, so Hero City places the ordinary friendly creatures, one beside the vent bounce. Every Blender model's exact-version review remains open.
