...
3. **Dispatch a top-tier Blender agent.** Every Blender task uses the driving provider's top tier with an empty context and a self-contained work order: `gpt-6-astra` at `max` from a Codex lead, never Sol; `claude-opus-5-5` from a Claude lead (Tom, September 26). Without a generated concept, the agent first renders a labeled Blender reference sheet, and the review says so. Supply actual image files, art-brief version, scale/orientation, required clips, budgets, artifact conventions and acceptance views. Grant exclusive ownership of the remote scene. Another asset can be sketched or coded concurrently, but another agent cannot mutate that scene.
