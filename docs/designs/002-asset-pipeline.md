...
Use the authoring workflow to build the PoC's avatar, materials, rig, and animation set. Publish validated versions under a stable avatar-asset identity. The same avatar can explore any configured person's memories; adding a person with usable photos requires no new art.
