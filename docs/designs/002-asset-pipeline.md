...
The [Blender runbook](https://github.com/thaynes43/haynes-ops/blob/main/.agents/runbooks/blender-authoring.md) is authoritative for pins, supported operations and artifact transfer. [PLAN-003](../../.agents/plans/003-authoring-tool-setup.md) records successful live scene, viewport, GLB validator, checksum and persistence checks. Native registration is staged pending the single dev-env activation; setup meshes are not approved game assets. Never edit generated MCP configuration or restart dev-env for routine authoring.
