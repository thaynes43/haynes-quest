...
Tom confirmed **image generation for asset sketches, followed by Blender controlled through MCP**, with his review before final assets are used. This is the agreed workflow for the game's shared avatar, enemy/boss catalog, and world art. PLAN-003 now places the pinned Blender LTS/ahujasid bridge in a dedicated cluster pod with Xvfb/software OpenGL, its own persistent workspace, and an internal streamable HTTP MCP endpoint. Blender and bridge share paths inside that pod; dev-env transfers artifacts explicitly. This follows Tom's instruction to upgrade authoring services independently of the development session. CPU rendering is the setup baseline; optional GPU render Jobs can use separately evaluated cluster hardware. Live service validation and initial agent registration precede authoring; later game code can use placeholders.
...
The pinned setup trial uses [ahujasid/blender-mcp](https://github.com/ahujasid/blender-mcp/tree/5f8ddaf6e987c4aa0c3467fcc548838b28f64477). Its [MCP implementation](https://github.com/ahujasid/blender-mcp/blob/5f8ddaf6e987c4aa0c3467fcc548838b28f64477/src/blender_mcp/server.py) supports scene/object inspection, viewport screenshots, and Python execution. This supplies the build/inspect/adjust loop; Blender's Python operators provide the modeling and export operations. It is a third-party bridge, not a model-generation guarantee. The [haynes-ops authoring runbook](https://github.com/thaynes43/haynes-ops/blob/main/.agents/runbooks/blender-authoring.md) is the operational reference to update with the dedicated service's pins and verification commands. Historical dev-env image 0.6.0 passed offline smoke and was published under haynes-ops #2831. Its Blender activation is superseded; the dedicated service and remote connection remain unverified. PLAN-003 retains the historical digest and checks without treating them as remote readiness.

Record these requirements for the dedicated service:

- **Blender host and display:** the candidate's [add-on](https://github.com/ahujasid/blender-mcp/blob/5f8ddaf6e987c4aa0c3467fcc548838b28f64477/addon.py) expects a running graphical Blender session and explicitly rejects background `-b` mode. Run Blender with Xvfb and software OpenGL beside the MCP bridge in the authoring pod. Separate background render/export Jobs can be useful later; they do not replace the interactive scene used by this bridge.
- **Transport boundary:** the upstream bridge defaults to stdio and communicates with the add-on over a raw TCP socket, normally port 9876. Adapt the pinned MCP server to serve streamable HTTP `/mcp` within the dedicated workload; verify protocol initialization and tool calls over that transport. Keep the raw add-on bound to `127.0.0.1:9876` and expose only the internal MCP Service to authorized clients through Cilium policy. No public ingress is needed. The bridge can execute Python, so it is an authoring control endpoint rather than a player-facing service.
- **Files and transfer:** the [screenshot implementation](https://github.com/ahujasid/blender-mcp/blob/5f8ddaf6e987c4aa0c3467fcc548838b28f64477/src/blender_mcp/server.py#L461) writes on Blender's filesystem and reads from the MCP process's filesystem. Colocation preserves that shared-path contract inside the service and lets MCP return image bytes. Use a separate authoring PVC for source references, `.blend` masters, renders, and exports. Define controlled upload/download and remote workspace paths; never assume the service can read a path in an agent worktree. Verify transferred artifact checksums and retain editable source alongside export metadata.
- **Private workspace:** constrain artifact access to authoring clients and permitted workspace paths. Configure `DISABLE_TELEMETRY=true` and verify it before private art, following the [upstream telemetry documentation](https://github.com/ahujasid/blender-mcp/blob/5f8ddaf6e987c4aa0c3467fcc548838b28f64477/README.md). Private artifacts stay outside public repository/build output until their distribution and review permit promotion.
- **Registration and upgrades:** register the stable remote `/mcp` endpoint in haynes-ops's GitOps-managed `kubernetes/main/apps/dev/dev-env/app/resources/config/claude/mcp.json` for both agents. Revise held PR #2833 to registration only, without activating the historical Blender dev-env image. The initial ConfigMap change still restarts dev-env under its current reloader, so that one held draft waits for Tom's natural break. Subsequent Blender image/add-on/service changes roll only the dedicated workload through ordinary GitOps PRs. Never edit generated agent configuration directly.
- **First connection check:** remotely inspect a synthetic scene, create an object, retrieve viewport bytes, save/reopen its editable file, and export/validate/transfer/load a GLB. Restart the authoring pod and confirm persistence and reconnect while the development session stays running. Validate both agents after initial registration. These are dependency checks, separate from later game-engine and actual-device tests.
- **GPU options:** Tom has a server capable of dual RTX 3090s and smaller GPUs in the cluster. Inventory availability, drivers, scheduling, and memory needs before assigning hardware. Keep the first interactive smoke on CPU/software graphics; add isolated GPU render Jobs if measured render workloads justify them. Audio model inference belongs in a separate service/Job under DESIGN-008, with its own dependencies and resource budget. No GPU allocation or performance is verified yet.
