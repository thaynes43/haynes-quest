...
Tom confirmed **image generation for asset sketches, followed by Blender controlled through MCP**, with his review before final assets are used. This is the agreed workflow for the game's shared avatar, enemy/boss catalog, and world art. PLAN-003 selects a pinned Blender LTS/ahujasid bridge trial inside dev-env under Xvfb/software OpenGL, with identical local paths for screenshots and editable files. Image smoke validation and a separately held runtime activation precede live authoring; later game code can use placeholders.
...
- **Blender host and display:** the candidate's [add-on](https://github.com/ahujasid/blender-mcp/blob/5f8ddaf6e987c4aa0c3467fcc548838b28f64477/addon.py) expects a running Blender session and explicitly rejects background `-b` mode. The current trial uses the dev-env pod with Xvfb and software OpenGL; validate that graphical path in the image smoke test before activation. This restriction is specific to this interactive bridge; Blender's separate background export capability remains useful for repeatable asset builds.
- **Transport and files:** its MCP process uses stdio and communicates with the add-on over a TCP socket, defaulting to loopback port 9876. The [screenshot implementation](https://github.com/ahujasid/blender-mcp/blob/5f8ddaf6e987c4aa0c3467fcc548838b28f64477/src/blender_mcp/server.py#L461) writes on Blender's filesystem and reads from the MCP process's filesystem. The current trial colocates both processes in dev-env, binding the add-on to 127.0.0.1:9876 and using the home PVC for shared files. No separate network service or screenshot-transfer path is required. Validate the identical-path assumption with the image and live smoke tests.
