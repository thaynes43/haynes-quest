...
Normal attack targeting always selects hostile encounters and never silently switches to a nearby friend. A separate deliberate interaction warns the player before sending an attack against a friend. Conversations open only when the player is grounded and close enough; the client checks the current position and pauses it immediately, so a stale prompt during a jump cannot trap the player in an unusable dialog. Unavailable actions show feedback. Equipped weapon damage can hurt or defeat them; defeat is a cartoon collapse with no gore. On first harm, the player loses two health points, stopping at one, and that friend's healing is unavailable until the friendship is repaired.
