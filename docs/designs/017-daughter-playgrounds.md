...
Every `jump` or `ride` connection also needs one common avatar-width lane with a clear 0.75m approach inside the source exit and destination entry. The lane starts one avatar radius inside each edge. Moving platforms must retain that common lane across their full motion; standing-height sweepers are checked across their entire swept footprint. This conservative v1 rule deliberately rejects narrow corner-only approaches and prevents a landing from requiring a last-instant jump. It checks safe entry and exit areas, not the whole flight trajectory or a general obstacle-avoidance solution. The actual-physics edge tests and full browser journey remain required.

Besties retreat resets the local trick sequence while preserving damage already dealt. Its routine also activates whenever the current attack tool can reach an actor. The authored court must contain the translated routine's full hazard footprint; this particular court is covered by the simulation tests, while a future relocation still needs that footprint check alongside document validation.
