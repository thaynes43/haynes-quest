...
PLAN008 final review also separates the pickup picture from later feedback by updating each notice as one text/photo value. Replacing a pickup notice with Attack, gear or friendly feedback clears its picture immediately, including when the old timer was canceled. Touch/pen sound unlock waits for release; mouse pointerdown remains valid. The sound-test confirmation describes startup, while physical listening remains a device check.

Pointer timestamps determine the 0–500 ms world-tap window. A real Chromium trace captured an 11 ms touch whose release handler arrived roughly 531 ms after its press handler; processing-time measurement incorrectly rejected it. Slow frames must not turn a quick physical tap into a long press. True gestures over 500 ms, dragging and cancellation retain their existing behavior. Passive nearby-gear hints sit above combat controls and cannot intercept touches.

A hosted PLAN008 retry exposed another pause boundary: network/model work may finish while a modal freezes the simulation. UI status must advance on raw frame time independently of physics, so a failed artwork retry either clears or becomes retryable without closing the modal or requiring a second press. The status interval remains bounded; paused movement, combat and animation time remain frozen.
