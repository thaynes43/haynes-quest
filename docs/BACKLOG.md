...
- **Status:** The fictional editor and shared agent CLI are deployed under [PLAN012](../.agents/plans/completed/012-level-editor-mvp.md) and [PLAN014](../.agents/plans/014-complete-world-builder.md). Parent admission, shared publication and new approved-art integration remain future work.
...
The next stage needs an owner-scoped promotion path for validated exported worlds, approved exact model and audio versions, parent-selected media and immutable published revisions. A new Rat Casino catalog/period must supply explicit date eligibility, editor-ready identities, motion/height mapping and environment theme/kit mapping. The catalog entry must accept the Rat Pit Boss's exact `v002` model; the current asset-version type allows only `v001`. When the catalog is extended, keep existing v2 project exports readable by resolving their pinned catalog version rather than requiring only the newest version. Do not attach Rat Casino studio candidates to encounters before Tom reviews their exact versions and the later progression/device gates pass. Preserve source dates and private media access throughout publication.
