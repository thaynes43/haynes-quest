...
| T-02 | Source photo | A photo obtained from a configured photo connection for game content or references during developer asset authoring. |
...
| T-07 | Playable character | A game character linked to a configured person and assigned an authored asset for the family PoC. It has a stable game-owned identity, distinct from the signed-in player, the person's display name, and the current model version. |
