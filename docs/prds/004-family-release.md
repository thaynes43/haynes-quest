...
| Q-06 | Codex (Astra) hit its usage limit on September 26, resetting September 30. Who makes the remaining models? | Claude Opus 5.5 Blender authors, one scene lease at a time, with the same validation and catalog steps. Tom clarified that the old Astra-only rule meant only Astra over Sol (Tom, September 26). |
| Q-07 | What do models without an Astra concept follow? | Each author first builds a painted front, side and back Blender reference sheet from the written brief, which the coordinator reviews before detailing. The review page labels it "Blender reference sheet, no generated concept" (Tom, September 26). |
