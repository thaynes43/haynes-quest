...
1. Update `docs/assets/reviews/<asset-id>/<version>.md` with exact sources, inspiration images, matching renders, available model/animation or audio previews, downloads, checksums, provenance and actual limitations. Keep earlier versions and working deep links. A revised concept must not imply that an unchanged model already incorporates it. For Blender GLBs, inspect exported scene extras before hashing: the work order and candidate status must name the current asset/version, not metadata inherited from the previous author in the shared scene.
2. Update `scripts/assets/catalog-inventory.json`: identities, versions, source/media paths, checksums, category, state and counts. Record `gameplay_use: "private-candidate"` for assets included in the private test; this records use, not final approval. Account for every delivered asset; each separately reviewable model in a kit needs its own entry. Retained historical files stay linked from the review history.
