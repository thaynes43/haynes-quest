...
The agreed visual workflow is image-generated sketches followed by Blender through MCP. Tom reviews final visual and audio assets before they enter gameplay. The [audio proposal](docs/designs/008-audio-pipeline.md) now recommends a self-hosted Stable Audio Small-SFX trial, with ElevenLabs as an optional free/paid hosted alternative. Both use FFmpeg for preparation and Web Audio playback; audio access and trial results are still pending. Automatic person-specific character generation remains [conditional future backlog](docs/BACKLOG.md#bl-01-automatic-playable-character-generation).

The immediate focus is the [core development loop](docs/designs/007-poc-development-loop.md): one test level, three synthetic memories, a new ability that builds on the starting actions, and save/resume. Fresh-context Astra agents coordinate code, assets, audio, design, and verification through self-contained work orders. Dependency setup is the current checkpoint before development dispatch. After setup, placeholder geometry lets gameplay progress while final candidate assets are produced and reviewed. Detailed combat, other collectibles, and the full story are deferred until this loop can be evaluated.
