/** Exact-byte Khronos validation and WO051 resource/clip contract. */
...
const root=process.argv[2]||'/workspace/haynes-quest/bickering-besties/v001';
const names=process.argv.slice(3);if(!names.length)throw Error('One explicit asset name is required.');
let failed=false;
for(const name of names){
 ...
 const checks={gltf_2:gltf.asset.version==='2.0',khronos_no_errors:report.issues.numErrors===0,khronos_no_warnings:report.issues.numWarnings===0,
  eight_exact_clips:clips.map(c=>c.name).sort().join()===['idle','move','attack','hit','defeat','cheer','high-five','dizzy'].sort().join(),
  ...
  all_have_uvs:prim.every(p=>p.attributes.TEXCOORD_0!==undefined),single_skin:gltf.skins.length===1,
