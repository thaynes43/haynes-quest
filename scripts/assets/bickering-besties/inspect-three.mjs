...
const names=process.argv.slice(4);if(!names.length)names.push('bestie-pink','bestie-black');
const specs={'bestie-pink':[1.4,0,null],'bestie-black':[1.4,0,null]};
let failed=false;
for(const name of names){
 decodedImages.length=0;
 const folder=path.join(media,name,'v001'),bytes=await readFile(path.join(folder,name+'.glb'));
 const construction=JSON.parse(await readFile(path.join(folder,'construction.json')));const contact=construction.clips.find(c=>c.name==='attack');
 ...
 const [h,f,w]=specs[name];const checks={embedded_pigment_decoded:decodedImages.length>0&&decodedImages.every(i=>i.width===1024&&i.height===1024&&i.decoded_bytes===4194304),six_or_fewer_draw_primitives:skinned.length<=6,skinned_geometry_loaded:skinned.length>0,
  ...
  exact_clip_set:Object.keys(animations).sort().join()===['idle','move','attack','hit','defeat','cheer','high-five','dizzy'].sort().join(),
  all_clips_move_actual_vertices:Object.values(animations).every(a=>a.max_vertex_displacement_m>.0005),
  all_loop_seams:['idle','move','cheer','dizzy'].every(name=>animations[name].loop_seam_max_m<.0003),
 ...
 checks.exact_contact_timing=Math.abs(attack.duration*contact.contact_fraction-contact.contact_time_s)<1e-7;
 ...
 const record={asset_id:name,tool:'Three GLTFLoader + AnimationMixer + actual sampled skinned vertices + precise bounds',three_revision:THREE.REVISION,
  ...
  attack_contact:{duration_s:attack.duration,time_s:contact.contact_time_s,fraction:contact.contact_fraction},checks,limitations:'CPU loader, fully decoded embedded pigment and geometry evidence; no WebGL raster, physical Safari or frame-time claim.'};
