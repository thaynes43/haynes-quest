"""WO100 bounded source transfer and execution through the supported MCP route."""
from pathlib import Path
from urllib.request import Request,urlopen
import base64,hashlib,json,sys

LOCAL=Path(__file__).resolve().parent
REMOTE='/workspace/haynes-quest/rat-casino-cast/chick-flia/v001'
ENDPOINT='http://blender-authoring.dev.svc.cluster.local:8000/mcp'

class MCP:
 def __init__(self):
  self.session=None;self.counter=0
  self.call('initialize',{'protocolVersion':'2024-11-05','capabilities':{},'clientInfo':{'name':'wo100-scoped-transfer','version':'1'}})
...
if __name__=='__main__':
 client=MCP()
 client.execute("import bpy; assert bpy.context.scene.get('work_order')=='WO100' and bpy.context.scene.get('scene_lease')=='active'")
