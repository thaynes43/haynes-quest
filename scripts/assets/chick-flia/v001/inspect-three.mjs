/** WO100 exact GLTFLoader + actual EnemyAnimation + complete skinned bounds.
 * Run with pnpm exec tsx scripts/assets/chick-flia/v001/inspect-three.mjs <artifact-dir>
 */
...
const root=path.resolve(process.argv[2]),bytes=await readFile(path.join(root,'chick-flia.glb'));
...
function points(){
 ...
 const positions=[],groups={feet:[],microphone:[],chest:[],head:[]};
 for(const mesh of meshes){
  ...
  for(let i=0;i<mesh.geometry.attributes.position.count;i++){
   ...
   if(bone==='prop_microphone')groups.microphone.push(p);
   if(bone==='chest')groups.chest.push(p);if(bone==='head'||bone==='jaw')groups.head.push(p);
  }
 }
...
let maxMicrophoneDrift=0,rootFixed=true,skinWeightsRigid=true,microphoneSuitIntersections=0,microphoneHeadIntersections=0;
...
const microphone=scene.getObjectByName('prop_microphone'),microphoneRest=microphone.matrix.clone();
for(const clip of gltf.animations){
 ...
 const total=Math.ceil(clip.duration*60),bounds=new T.Box3();let first,last,hold,heldDifference=0,maxDisplacement=0,maxTemporalDisplacement=0,feetMin=Infinity,microphoneMin=Infinity;
 for(let i=0;i<=total;i++){
  ...
  if(i===0)first=sample.positions;if(i===total)last=sample.positions;for(let j=0;j<sample.positions.length;j++)maxTemporalDisplacement=Math.max(maxTemporalDisplacement,sample.positions[j].distanceTo(first[j]));
  ...
  for(const p of sample.groups.microphone)microphoneMin=Math.min(microphoneMin,p.y);
  const chestBox=new T.Box3().setFromPoints(sample.groups.chest);
  microphoneSuitIntersections+=sample.groups.microphone.filter(p=>chestBox.containsPoint(p)).length;const headBox=new T.Box3().setFromPoints(sample.groups.head);microphoneHeadIntersections+=sample.groups.microphone.filter(p=>headBox.containsPoint(p)).length;
  scene.updateMatrixWorld(true);maxMicrophoneDrift=Math.max(maxMicrophoneDrift,...microphone.matrix.elements.map((x,j)=>Math.abs(x-microphoneRest.elements[j])));
 ...
 animations[clip.name]={duration_s:clip.duration,samples:total+1,all_vertices_per_sample:rest.positions.length,max_vertex_displacement_m:maxDisplacement,max_temporal_vertex_displacement_m:maxTemporalDisplacement,bounds_y_up:pack(bounds),loop_seam_max_m:seam,feet_min_y_m:feetMin,microphone_min_y_m:microphoneMin,held_defeat_difference_m:clip.name==='defeat'?heldDifference:null};union.union(bounds);
}
mixer.stopAllAction();const contactAction=mixer.clipAction(gltf.animations.find(c=>c.name==='attack')).reset().setLoop(T.LoopOnce,1);contactAction.clampWhenFinished=true;contactAction.play();mixer.setTime(1.25);scene.updateMatrixWorld(true);const directContactHand=scene.getObjectByName('hand_L').getWorldPosition(new T.Vector3());
...
const frame={id:'wo100-inspection',position:{x:0,y:0,z:0},facing:0,phase:'idle',windupProgress:0,hp:100,maxHp:100};
const phases=[];
...
const storedBounds=scene.getObjectByName('Chick_Flia_Skin').userData.model_space_bounds_y_up;
...
const checks={exported_bounds_cover_all_poses:!!storedBounds&&union.min.toArray().every((v,i)=>v>=storedBounds.min[i])&&union.max.toArray().every((v,i)=>v<=storedBounds.max[i]),all_five_clips:gltf.animations.map(c=>c.name).sort().join()===['idle','move','attack','hit','defeat'].sort().join(),all_tracks_resolve_below_scene_root:bindings.every(b=>b.resolves&&!b.targets_scene_root),
 ...
 no_floor_penetration:Object.values(animations).every(a=>a.bounds_y_up.min[1]>=-.00001),microphone_clear_of_suit:microphoneSuitIntersections===0,microphone_clear_of_face:microphoneHeadIntersections===0,microphone_attachment_fixed:maxMicrophoneDrift<.00001,all_rigid_single_weight:skinWeightsRigid,
 identity_root_no_motion:rootFixed,no_root_bone_motion:rootBoneTracks.every(t=>t.constant),correct_height:Math.abs(rest.box.max.y-1.72)<.00001,floor_centered_rest:Math.abs(rest.box.min.y)<.00001&&Math.abs(rest.box.max.x+rest.box.min.x)<.15,
...
adapter.dispose();
const record={asset_id:'chick-flia',version:'v001',glb_sha256:createHash('sha256').update(bytes).digest('hex'),three_revision:T.REVISION,method:'Exact GLTFLoader, all exported skin vertices at 60 Hz, SkeletonUtils clone and actual src/game/enemy-animation.ts',rest_bounds_y_up:pack(rest.box),all_animation_bounds_y_up:pack(union),dimensions_m:rest.box.getSize(new T.Vector3()).toArray(),exported_safe_bounds_y_up:storedBounds,animations,images,root_bone_tracks:rootBoneTracks,tracks:bindings,microphone_max_local_matrix_drift:maxMicrophoneDrift,microphone_vertices_inside_chest_bounds:microphoneSuitIntersections,microphone_vertices_inside_head_bounds:microphoneHeadIntersections,adapter:{phases,direct_contact_hand:directContactHand.toArray(),hit_hand:hitHand,defeat_states:defeatStates},checks};
await writeFile(path.join(root,'three-inspection.json'),JSON.stringify(record,null,2)+'\n');
const failures=Object.entries(checks).filter(([,v])=>!v).map(([k])=>k);console.log(JSON.stringify({failures,rest:record.rest_bounds_y_up,animated:record.all_animation_bounds_y_up,animations,microphone_max_drift:maxMicrophoneDrift,microphone_suit_intersections:microphoneSuitIntersections}));process.exitCode=failures.length?1:0;
