/** WO-016 isolated real Chromium/WebGL intake; no shared browser or live scene.
...
const names=['loop-dancer','prism-mimic','trendweaver'];
...
window.loadAsset=async(name,height)=>{
 ...
 camera.position.set(2.0,height*.95,-4.5);camera.lookAt(0,height*.50,0);camera.updateProjectionMatrix();
...
try{
 ...
 for(const name of names){
  ...
  await page.evaluate(fraction=>window.showClip('attack',fraction),name==='trendweaver'?.6:.5);await page.screenshot({path:path.join(folder,'browser-attack.png')});
