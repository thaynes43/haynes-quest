/** Exact-byte Khronos validation and WO-016 resource/clip contract. */
...
const root=process.argv[2]||'/workspace/haynes-quest/era-2024/v001';
const names=process.argv.slice(3);if(!names.length)names.push('loop-dancer','prism-mimic','trendweaver');
let failed=false;
for(const name of names){
 ...
 const checks={gltf_2:gltf.asset.version==='2.0',khronos_no_errors:report.issues.numErrors===0,khronos_no_warnings:report.issues.numWarnings===0,
  ...
  all_have_vertex_colors:prim.every(p=>p.attributes.COLOR_0!==undefined),single_skin:gltf.skins.length===1,
