"""WO-016 views and motion rendered from exact re-imported GLBs.
...
def render(output,name,quick=False,stills=False,video=False):
    ...
    h={'loop-dancer':1.10,'prism-mimic':1.05,'trendweaver':1.90}[name]
    scale={'loop-dancer':1.55,'prism-mimic':1.70,'trendweaver':2.60}[name]
    target=(0,0,h*.50)
    cam=studio(h,width=700,height_px=800,samples=24)
    if quick or stills:
    ...
    if video:
        ...
        subprocess.run(['ffmpeg','-hide_banner','-loglevel','error','-y','-framerate','12','-i',str(turns/'%03d.png'),'-vf','fps=24','-c:v','libx264','-crf','22','-pix_fmt','yuv420p','-movflags','+faststart',str(folder/'turntable.mp4')],check=True)
...
if __name__=='__main__':
    ...
    parser.add_argument('--output',default='/workspace/haynes-quest/era-2024/v001')
