"""WO098 deterministic baked mechanical rig, floor targets and exact enemy clips."""
...
def evaluate(clip,t):
 ...
 feet={s:Vector(REST['foot_'+s][0]) for s in ('L','R')}
 ...
 if clip=='idle':
  ...
  for label,sgn in [('R',-1),('L',1)]:
   ...
   feet[label]+=Vector((0,math.cos(phase)*.069,max(0,step)*.062))
  hands['R']+=Vector((.008*sw,.015*sw,.032+.022*(1-co)))
  hands['L']+=Vector((.020*sw,-.063*co,-.048+.015*sw));hand_angle=.10*sw
 elif clip=='attack':
  rest=REST['hand_L'][0];wind=(.39,-.070,1.49);contact=(.285,.448,1.295)
  hands['L']=keyed(t,[(0,rest),(.42,wind),(.54,wind),(.625,contact),(.69,contact),(.86,(.49,.24,1.20)),(1,rest)])
  windup=pulse(t,.43,.43);strike=pulse(t,.625,.24)
  hips_shift.z-=.022*strike;body_rot.x=.038*windup-.080*strike
  body_rot.z=.045*windup-.065*strike;head_rot.x=.06*windup-.045*strike;head_rot.z=-.05*windup
  hands['R'].z+=.022*strike;hand_angle=-.08*windup+.08*strike
  ...
  hips_shift+=Vector((0,-.035*settle,-.205*settle))
  body_rot=Vector((-.18*settle+.10*startle,0,.045*settle))
  head_rot=Vector((-.30*settle+.15*startle,.11*settle,-.09*settle))
  hands['L']=lerp(hands['L'],(.405,.185,.795),settle)+Vector((0,0,.15*startle))
  hands['R'].z+=.04*startle;hand_angle=-1.65*settle
 ...
 for j in range(1,4):inherited('tail_'+str(j),(0,tail_sway*.3,tail_sway*(.4+j*.15)))
...
for name,duration,loop in CLIPS:
 ...
 records.append({'name':name,'duration_s':duration,'loop':loop,'clamp_when_finished':not loop,'contact_time_s':1.25 if name=='attack' else None,'contact_fraction':.625 if name=='attack' else None,'held_final_pose_from_s':1.76 if name=='defeat' else None})
...
for name in ('build.py','common.py','animate.py'):
 old=bpy.data.texts.get('WO098 '+name)
 if old:bpy.data.texts.remove(old)
 block=bpy.data.texts.new('WO098 '+name);block.write((ROOT/'source'/name).read_text())
...
record['rig']='One skin with rigid metal plate islands, one joint influence per vertex. Analytic two-bone limb targets bake to local transforms; level feet and a vertical hand-attached cane. Root remains stationary; no runtime constraints.'
...
record['initial_checkpoint_corrections']=['Lowered floating hat brim by 0.042 construction units; resized complete model to 2.15 m including hat.','Deepened burgundy pigment to match the reference.','Reduced decorative geometry and retained named source parts; final two-material skin has 14,144 triangles.']
