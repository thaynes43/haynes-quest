/** WO104 exact-byte Khronos validation and provisional character budgets. */
...
const root=path.resolve(process.argv[2]),file='golden-after-hours-rat.glb',data=await readFile(path.join(root,file));
...
const record={asset_id:'golden-after-hours-rat',version:'v001',file,sha256:createHash('sha256').update(data).digest('hex'),bytes:data.length,triangles,draw_primitives:primitives.length,
