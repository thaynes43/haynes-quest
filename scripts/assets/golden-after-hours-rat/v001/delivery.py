"""WO104 durable artifact intake, supported MCP transfer, hashes and release.
...
LOCAL=Path('/home/dev/artifacts/haynes-quest/rat-casino-cast/golden-after-hours-rat/v001')
SOURCE=Path(__file__).resolve().parent
BASE='http://blender-authoring.dev.svc.cluster.local:8000/artifacts/haynes-quest/rat-casino-cast/golden-after-hours-rat/v001/'
...
if sys.argv[1]=='stage':
 ...
 for name in ['golden-after-hours-rat.blend','golden-after-hours-rat.glb','pigment.png','construction.json','validation.json','source/rig-rest.json']:fetch(name)
 reports=[json.loads((LOCAL/name).read_text()) for name in ['validation.json','three-inspection.json','browser-inspection.json']]
 expected=sha((LOCAL/'golden-after-hours-rat.glb').read_bytes())
 ...
 for name in ['scene-claim.json','scene-release.json','live-scene-release.blend']:fetch(name)
 assert json.loads((LOCAL/'scene-release.json').read_text())['scene_lease']=='released'
 names=['golden-after-hours-rat.blend','golden-after-hours-rat.glb','pigment.png','construction.json','validation.json','three-inspection.json','browser-inspection.json','visual-review.json','provenance.json','source-prompt.txt','tool-settings.json','scene-claim.json','scene-release.json','live-scene-release.blend','front.png','side.png','back.png','beauty.png','detail.png','hinge.png','motion-contact-sheet.png','attack-strip.png','attack-gameplay-scale.png']
 ...
 record={'work_order':'WO104','asset_id':'golden-after-hours-rat','version':'v001','created_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'local_root':str(LOCAL),'remote_root':REMOTE,'scene_lease':'released','candidate_status':'Validated studio candidate; exact owner review pending; no gameplay mapping','files':files,'all_remote_local_hashes_match':True,'verification_method':'MCP SHA-256 for source/script/text extensions (artifact HTTP route does not serve them); HTTP byte roundtrip for media, JSON and masters.','excluded_checkpoints':['early-preview/','revised-preview/','revised-wear-preview/','golden-after-hours-rat-checkpoint.glb','golden-after-hours-rat-construction.blend'],'manifest_self_hash':'Excluded to avoid recursive hashing.'}
 (LOCAL/'sha256-manifest.json').write_text(json.dumps(record,indent=2)+'\n');upload('sha256-manifest.json')
 print(json.dumps({'stage':'complete','files':len(files),'manifest_sha256':sha((LOCAL/'sha256-manifest.json').read_bytes()),'scene_lease':'released','glb_sha256':sha((LOCAL/'golden-after-hours-rat.glb').read_bytes())}))
