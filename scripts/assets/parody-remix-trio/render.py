"""WO-032 views and motion rendered from exact re-imported GLBs.
...
def encode_previews(folder, clips):
    """Thirty actual GLB samples per second; integer media frames preserve exact clip seconds."""
    folder=Path(folder)
    for clip,duration in clips:
        frames=folder/('frames-'+clip)
        subprocess.run(['ffmpeg','-hide_banner','-loglevel','error','-y','-framerate','30','-i',str(frames/'%03d.png'),'-c:v','libx264','-preset','medium','-crf','22','-pix_fmt','yuv420p','-movflags','+faststart',str(folder/(clip+'.mp4'))],check=True)
...
def render(output,name,quick=False,stills=False,video=False):
    ...
    clear();bpy.context.scene.render.fps=24;arm=load(folder/(name+'.glb'))
    ...
    h=json.loads((folder/'construction.json').read_text())['spec']['height']
    scale=h*1.27
    target=(0,0,h*.50)
    cam=studio(h,width=900 if not quick else 700,height_px=1000 if not quick else 800,samples=32 if not quick else 10)
    if quick or stills:
        shot(folder/('quick.png' if quick else 'beauty.png'),cam,(2.8*h,5*h,2.4*h),target,scale)
    if quick:
        shot(folder/'checkpoint-front.png',cam,(0,5*h,h*.50),target,scale*.94)
        shot(folder/'checkpoint-back.png',cam,(0,-5*h,h*.50),target,scale*.94)
        (folder/'checkpoint-complete.json').write_text(json.dumps({'sha256':inspection['glb_sha256'],'completed_unix':time.time()}))
        return
    if stills:
        shot(folder/'front.png',cam,(0,5*h,h*.50),target,scale*.94)
        shot(folder/'side.png',cam,(5*h,0,h*.50),target,scale*.94)
        shot(folder/'back.png',cam,(0,-5*h,h*.50),target,scale*.94)
    if video:
        sc=bpy.context.scene;sc.render.resolution_x=480;sc.render.resolution_y=560;sc.cycles.samples=8
        cam.location=(2.8*h,5*h,2.4*h);cam.rotation_euler=(Vector(target)-cam.location).to_track_quat('-Z','Y').to_euler();cam.data.ortho_scale=scale*1.12
        expected=['idle','move','attack','hit','defeat']
        for clip in expected:
            frames=folder/('frames-'+clip);frames.mkdir(exist_ok=True)
            action=next(a for a in bpy.data.actions if a.name==clip or a.name.startswith(clip+'_'))
            start,end=action.frame_range
            for i in range(round((end-start)*30/24)):
                set_pose(arm,clip,start+i*24/30)
        ...
        for i in range(48):
            angle=math.tau*i/48
            shot(turns/('%03d.png'%i),cam,(5*h*math.sin(angle),5*h*math.cos(angle),2.4*h),target,scale*1.12)
        subprocess.run(['ffmpeg','-hide_banner','-loglevel','error','-y','-framerate','16','-i',str(turns/'%03d.png'),'-vf','fps=30','-c:v','libx264','-crf','22','-pix_fmt','yuv420p','-movflags','+faststart',str(folder/'turntable.mp4')],check=True)
    set_pose(arm)
    cam.location=(2.8*h,5*h,2.4*h);cam.rotation_euler=(Vector(target)-cam.location).to_track_quat('-Z','Y').to_euler();cam.data.ortho_scale=scale
...
if __name__=='__main__':
    ...
    parser.add_argument('--output',default='/workspace/haynes-quest/parody/remix-trio/v001')
