...
Requires an explicit active WO032 lease. No scene objects or addons are changed.
...
REMOTE='/workspace/haynes-quest/parody/remix-trio/v001'
ENDPOINT='http://blender-authoring.dev.svc.cluster.local:8000/mcp'
ARTIFACTS='http://blender-authoring.dev.svc.cluster.local:8000/artifacts/haynes-quest/parody/remix-trio/v001/'

class MCP:
 def __init__(self):
  self.session=None;self.counter=0
  self.call('initialize',{'protocolVersion':'2024-11-05','capabilities':{},'clientInfo':{'name':'wo032-scoped-file-transfer','version':'1'}})
...
def main():
 ...
 assert lease['owner']=='/root/remix_trio_authoring' and lease['status'].startswith('active'),lease
 ...
 if selected:
  for name in selected:
   if '/' in name:
    asset,file=name.split('/',1);assert asset in ['sir-flush-a-lot','nap-captain','one-star-diva'] and Path(file).name==file and file not in ['concept.png','prompt.txt','concept-provenance.json']
  ...
  for name in ['sir-flush-a-lot','nap-captain','one-star-diva']:
