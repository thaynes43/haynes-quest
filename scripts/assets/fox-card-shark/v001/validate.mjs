/** WO102 exact-byte Khronos validation and provisional character budgets. */
...
const root=path.resolve(process.argv[2]),file='fox-card-shark.glb',data=await readFile(path.join(root,file));
...
const record={asset_id:'fox-card-shark',version:'v001',file,sha256:createHash('sha256').update(data).digest('hex'),bytes:data.length,triangles,draw_primitives:primitives.length,
