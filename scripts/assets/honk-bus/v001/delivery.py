...
LOCAL=Path('/home/dev/artifacts/haynes-quest/family-eras/honk-bus/v001')
SOURCE=Path(__file__).resolve().parent
BASE='http://blender-authoring.dev.svc.cluster.local:8000/artifacts/haynes-quest/family-eras/honk-bus/v001/'
...
if sys.argv[1]=='stage':
 ...
 for name in ['three-inspection.json','visual-review.json','provenance.json','tool-settings.json','attachment-inspection.json','prompt.txt','source.json']:upload(name)
 for name in ['honk-bus.blend','honk-bus.glb','pigment.png','construction.json','validation.json','source/rig-rest.json','honk-bus-high-density-checkpoint.blend','honk-bus-construction.blend','honk-bus-checkpoint.glb']:fetch(name)
 ...
 expected=sha((LOCAL/'honk-bus.glb').read_bytes())
 ...
 names=['honk-bus.blend','honk-bus.glb','pigment.png','concept.png','prompt.txt','source.json','construction.json','validation.json','three-inspection.json','browser-inspection.json','visual-review.json','provenance.json','tool-settings.json','attachment-inspection.json','honk-bus-high-density-checkpoint.blend','honk-bus-construction.blend','honk-bus-checkpoint.glb','scene-lease.json','scene-release.json','live-scene-release.blend','front.png','side.png','back.png','beauty.png','motion-contact-sheet.png','attack-strip.png','attack-gameplay-scale.png']
 ...
 record={'work_order':'WO111','asset_id':'honk-bus','version':'v001','created_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'local_root':str(LOCAL),'remote_root':REMOTE,'scene_lease':'released','candidate_status':"Awaiting Tom's review · used in the family release",'files':files,'all_remote_local_hashes_match':True,'verification_method':'MCP SHA-256 for source/text extensions; HTTP byte roundtrip for media, JSON and masters.','excluded_checkpoints':['early-preview/','motion-preview/','defeat-preview/','pre-clearance-preview/','final-intake/','pre-final-bounds-run.json','bounds-run.json','final-bounds-run.json'],'manifest_self_hash':'Excluded to avoid recursive hashing.'}
 (LOCAL/'sha256-manifest.json').write_text(json.dumps(record,indent=2)+'\n');upload('sha256-manifest.json')
 print(json.dumps({'stage':'complete','files':len(files),'manifest_sha256':sha((LOCAL/'sha256-manifest.json').read_bytes()),'scene_lease':'released','glb_sha256':sha((LOCAL/'honk-bus.glb').read_bytes()),'master_sha256':sha((LOCAL/'honk-bus.blend').read_bytes())}))
