...
const root=path.resolve(process.argv[2]),file='honk-bus.glb',data=await readFile(path.join(root,file));
...
const checks={gltf_2:gltf.asset.version==='2.0',khronos_no_errors:report.issues.numErrors===0,khronos_no_warnings:report.issues.numWarnings===0,
 correct_scene_identity:gltf.scenes.every(s=>s.extras?.work_order==='WO111'&&s.extras?.asset_id==='honk-bus'&&s.extras?.asset_version==='v001'&&s.extras?.candidate_status?.startsWith('WO111 honk-bus v001')),
...
const record={asset_id:'honk-bus',version:'v001',file,sha256:createHash('sha256').update(data).digest('hex'),bytes:data.length,triangles,draw_primitives:primitives.length,
