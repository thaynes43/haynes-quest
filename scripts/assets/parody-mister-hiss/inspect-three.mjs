...
const names=process.argv.slice(4);if(!names.length)names.push('mister-hiss');
const specs={'mister-hiss':[1,0,null]};
let failed=false;
for(const name of names){
 ...
 checks.exact_contact_timing=Math.abs(attack.duration*.6-.9)<1e-7;
 ...
 const record={asset_id:name,tool:'Three GLTFLoader + AnimationMixer + actual sampled skinned vertices + precise bounds',three_revision:THREE.REVISION,
  ...
  attack_contact:{duration_s:attack.duration,time_s:.9,fraction:.6},checks,limitations:'CPU loader, fully decoded embedded pigment and geometry evidence; no WebGL raster, physical Safari or frame-time claim.'};
