/** Exact-byte Khronos validation and WO-027 resource/clip contract. */
...
const root=process.argv[2]||'/workspace/haynes-quest/parody/mister-hiss/v001';
const names=process.argv.slice(3);if(!names.length)names.push('mister-hiss');
let failed=false;
for(const name of names){
 const folder=root,file=name+'.glb',data=await readFile(path.join(folder,file));
