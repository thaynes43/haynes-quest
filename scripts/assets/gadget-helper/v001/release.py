...
assert sc.get('work_order')=='WO111' and sc.get('asset_id')=='gadget-helper' and sc.get('scene_lease')=='active'
...
glb_hash=sha(ROOT/'gadget-helper.glb')
...
record={'work_order':'WO111','asset_id':'gadget-helper','version':'v001','released_utc':now,'scene_owner':'none','scene_lease':'released','saved_live_scene':str(ROOT/'live-scene-release.blend'),'saved_live_scene_sha256':sha(ROOT/'live-scene-release.blend'),'editable_master':str(ROOT/'gadget-helper.blend'),'editable_master_sha256':sha(ROOT/'gadget-helper.blend'),'glb_sha256':glb_hash,'author_jobs_running':any(jobs.values()),'job_checks':jobs,'prior_masters_unchanged':lease['prior_masters'],'scope':"Awaiting Tom's review · used in the family release. Candidate technical delivery only; lead owns publication and runtime integration. No physical-device acceptance."}
