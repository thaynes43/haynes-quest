/** WO111 yes-yes-veggie exact GLB Chromium stills and later motion sampling; no shared browser. */
...
const c=new T.OrthographicCamera(-.6,.6,.675,-.675,.01,30);const g=await new GLTFLoader().loadAsync('/asset.glb');s.add(g.scene);const m=new T.AnimationMixer(g.scene);
window.draw=(view='beauty',clip=null,fraction=0,scale=1)=>{m.stopAllAction();if(clip){const a=m.clipAction(g.animations.find(a=>a.name===clip));a.setLoop(T.LoopOnce,1);a.clampWhenFinished=true;a.play();m.setTime(a.getClip().duration*fraction)}g.scene.updateMatrixWorld(true);const v={front:[0,.52,-6],side:[6,.52,0],back:[0,.52,6],beauty:[3.1,1.52,-6],detail:[-4,1.2,-5],hinge:[4,1.2,5]}[view];c.position.set(...v);c.lookAt(0,view==='detail'||view==='hinge'?.6:.52,0);c.zoom=scale*(view==='detail'||view==='hinge'?2.25:1);c.updateProjectionMatrix();r.render(s,c);return {clips:g.animations.map(a=>({name:a.name,duration:a.duration})),calls:r.info.render.calls,triangles:r.info.render.triangles,glError:r.getContext().getError(),webglVersion:r.getContext().getParameter(r.getContext().VERSION),pose:g.scene.getObjectByName('hand_L').getWorldPosition(new T.Vector3()).toArray()}};
