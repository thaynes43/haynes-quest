/** WO103 exact GLB Chromium stills and later motion sampling; no shared browser. */
...
window.draw=(view='beauty',clip=null,fraction=0,scale=1)=>{m.stopAllAction();if(clip){const a=m.clipAction(g.animations.find(a=>a.name===clip));a.setLoop(T.LoopOnce,1);a.clampWhenFinished=true;a.play();m.setTime(a.getClip().duration*fraction)}g.scene.updateMatrixWorld(true);const v={front:[0,.84,-6],side:[6,.84,0],back:[0,.84,6],beauty:[3.1,1.57,-6],detail:[4,1.17,-5],hinge:[4,1.17,5]}[view];c.position.set(...v);c.lookAt(0,view==='detail'||view==='hinge'?.91:.84,0);c.zoom=scale*(view==='detail'||view==='hinge'?2.25:1);c.updateProjectionMatrix();r.render(s,c);return {clips:g.animations.map(a=>({name:a.name,duration:a.duration})),calls:r.info.render.calls,triangles:r.info.render.triangles,glError:r.getContext().getError(),webglVersion:r.getContext().getParameter(r.getContext().VERSION),pose:g.scene.getObjectByName('hand_L').getWorldPosition(new T.Vector3()).toArray()}};
