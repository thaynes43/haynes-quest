"""WO-028 views and motion rendered from exact re-imported GLBs.
...
def inspect(arm,stage):
    ...
    for clip in expected:
        ...
        result['actions'][clip]={'duration_s':float(end-start)/40,'samples':17,
...
def encode_previews(folder, clips):
    """20fps previews of the exact 40fps clip, played at the actual GLB duration."""
    folder=Path(folder)
    for clip,duration in clips:
        frames=folder/('frames-'+clip)
        subprocess.run(['ffmpeg','-hide_banner','-loglevel','error','-y','-framerate','20','-i',str(frames/'%03d.png'),'-c:v','libx264','-preset','medium','-crf','22','-pix_fmt','yuv420p','-movflags','+faststart',str(folder/(clip+'.mp4'))],check=True)
        subprocess.run(['ffmpeg','-hide_banner','-loglevel','error','-y','-i',str(folder/(clip+'.mp4')),'-vf','fps=%s,scale=240:-1,tile=4x1'%(4/duration),'-frames:v','1',str(folder/('strip-'+clip+'.png'))],check=True)
...
def render(output,name,quick=False,stills=False,video=False):
    root=Path(output);folder=root/name
    ...
    dragon=name=='drama-dragon'
    h=1.8 if dragon else 1.15
    scale=3.70 if dragon else 1.42
    target=(0,-.30 if dragon else 0,h*.49)
    cam=studio(h,width=(1200 if dragon else 900) if not quick else (960 if dragon else 700),height_px=(850 if dragon else 1000) if not quick else (700 if dragon else 800),samples=24 if not quick else 8)
    if stills:
        shot(folder/'beauty.png',cam,(5.2,4.1,2.7) if dragon else (2.8,5,2.4),target,scale)
    if quick:
        shot(folder/'quick.png',cam,(5.2,4.1,2.7) if dragon else (2.8,5,2.4),target,scale)
        ...
        set_pose(arm,'defeat',inspection['actions']['defeat']['duration_s']*40)
        shot(folder/'pose-defeat.png',cam,(5.2,4.1,2.7) if dragon else (2.8,5,2.4),target,scale)
        set_pose(arm,'attack',inspection['actions']['attack']['duration_s']*40*.625)
        shot(folder/'pose-attack.png',cam,(5.2,4.1,2.7) if dragon else (2.8,5,2.4),target,scale)
        set_pose(arm)
        (folder/'checkpoint-complete.json').write_text(json.dumps({'sha256':inspection['glb_sha256'],'completed_unix':time.time()}))
        if not (stills or video):return
    if stills:
    ...
    if video:
        sc=bpy.context.scene;sc.render.resolution_x=640 if dragon else 400;sc.render.resolution_y=400 if dragon else 480;sc.cycles.samples=6
        cam.location=(5.2,4.1,2.7) if dragon else (2.8,5,2.4);cam.rotation_euler=(Vector(target)-cam.location).to_track_quat('-Z','Y').to_euler();cam.data.ortho_scale=scale*1.12
        expected=['idle','move','attack','hit','defeat']
        for clip in expected:
            frames=folder/('frames-'+clip);frames.mkdir(exist_ok=True)
            action=next(a for a in bpy.data.actions if a.name==clip or a.name.startswith(clip+'_'))
            start,end=action.frame_range
            for i in range(round((end-start)/2)):
                set_pose(arm,clip,start+i*2)
        ...
        for i in range(48):
            angle=math.tau*i/48
            shot(turns/('%03d.png'%i),cam,(5*math.sin(angle),5*math.cos(angle),2.7 if dragon else 2.4),target,scale*1.12)
        subprocess.run(['ffmpeg','-hide_banner','-loglevel','error','-y','-framerate','16','-i',str(turns/'%03d.png'),'-vf','fps=20','-c:v','libx264','-crf','22','-pix_fmt','yuv420p','-movflags','+faststart',str(folder/'turntable.mp4')],check=True)
    set_pose(arm)
    cam.location=(5.2,4.1,2.7) if dragon else (2.8,5,2.4);cam.rotation_euler=(Vector(target)-cam.location).to_track_quat('-Z','Y').to_euler();cam.data.ortho_scale=scale
    bpy.context.scene['exact_glb_sha256']=inspection['glb_sha256']
    bpy.ops.wm.save_as_mainfile(filepath=str(folder/(name+'-export-review.blend')))
...
if __name__=='__main__':
    ...
    parser.add_argument('--output',default='/workspace/haynes-quest/parody/block-party-pair/v001')
