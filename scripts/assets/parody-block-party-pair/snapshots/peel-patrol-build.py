...
def drama_dragon():
    bone('body',(0,-.12,.65));bone('neck',(0,.24,.88),'body');bone('head',(0,.49,1.28),'neck');bone('jaw',(0,.80,1.28),'head')
...
def animate_dragon(clip,t):
    ...
    elif clip=='defeat':settle=smooth((t-.18)/.60);drop=.29*settle;lean=.07*settle
    fk('body',rot=(lean,roll,0),loc=(0,0,lift-drop))
    fk('neck',rot=(-.025*wave if clip=='idle' else -.08*wind+.14*stomp+.12*settle,0,0))
    fk('head',rot=(.018*wave if clip=='idle' else -.11*wind+.09*stomp+.12*settle,.04*wave if clip in ['idle','hit'] else 0,0))
    ...
    for i in range(8):fk('tail_%02d'%i,rot=(.012*wave if clip=='idle' else .045*settle,0,.032*math.sin(math.tau*t-i*.45) if clip in ['idle','move'] else .025*stomp))
