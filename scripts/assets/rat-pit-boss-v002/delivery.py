"""WO099 durable artifact intake, supported MCP transfer, hashes and release.
...
Only the work order's v002 artifact paths and source directory are used.
"""
...
LOCAL=Path('/home/dev/artifacts/haynes-quest/rat-casino-cast/rat-pit-boss/v002')
SOURCE=Path(__file__).resolve().parent
BASE='http://blender-authoring.dev.svc.cluster.local:8000/artifacts/haynes-quest/rat-casino-cast/rat-pit-boss/v002/'
...
if sys.argv[1]=='stage':
 ...
 for name in ['three-inspection.json','visual-review.json']:upload(name)
 for name in ['rat-pit-boss.blend','rat-pit-boss.glb','pigment.png','construction.json','validation.json','source/rig-rest.json']:fetch(name)
 reports=[json.loads((LOCAL/name).read_text()) for name in ['validation.json','three-inspection.json','browser-inspection.json']]
 expected=sha((LOCAL/'rat-pit-boss.glb').read_bytes())
 ...
 names=['rat-pit-boss.blend','rat-pit-boss.glb','pigment.png','construction.json','validation.json','three-inspection.json','browser-inspection.json','visual-review.json','scene-lease.json','scene-release.json','live-scene-release.blend','front.png','side.png','back.png','beauty.png','motion-contact-sheet.png','attack-strip.png','attack-gameplay-scale.png']
 ...
 record={'work_order':'WO099','asset_id':'rat-pit-boss','version':'v002','created_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'local_root':str(LOCAL),'remote_root':REMOTE,'scene_lease':'released','candidate_status':'Validated studio candidate; exact owner review pending; no gameplay mapping','files':files,'all_remote_local_hashes_match':True,'verification_method':'MCP SHA-256 for source extensions; HTTP byte roundtrip for media, JSON and masters.','excluded_checkpoints':['early-preview/','corrected-preview/','attack-revision-preview/','rat-pit-boss-checkpoint.glb','pigment-source.png'],'manifest_self_hash':'Excluded to avoid recursive hashing.'}
 (LOCAL/'sha256-manifest.json').write_text(json.dumps(record,indent=2)+'\n');upload('sha256-manifest.json')
 print(json.dumps({'stage':'complete','files':len(files),'manifest_sha256':sha((LOCAL/'sha256-manifest.json').read_bytes()),'scene_lease':'released','glb_sha256':sha((LOCAL/'rat-pit-boss.glb').read_bytes())}))
