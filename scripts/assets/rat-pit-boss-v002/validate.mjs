/** WO099 exact-byte Khronos validation and provisional character budgets. */
...
const root=path.resolve(process.argv[2]),file='rat-pit-boss.glb',data=await readFile(path.join(root,file));
...
const record={asset_id:'rat-pit-boss',version:'v002',file,sha256:createHash('sha256').update(data).digest('hex'),bytes:data.length,triangles,draw_primitives:primitives.length,
