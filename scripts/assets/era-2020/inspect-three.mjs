...
const {default:sharp}=await import(pathToFileURL(path.join(path.dirname(three),'sharp/dist/index.mjs')).href);
const decodedImages=[];
globalThis.self=globalThis;
// GLTFLoader needs a browser-like image endpoint even for CPU geometry intake.
// Decode every real embedded image with Sharp; retain dimensions and RGBA bytes.
globalThis.createImageBitmap=async blob=>{
 const input=Buffer.from(await blob.arrayBuffer());const {data,info}=await sharp(input).ensureAlpha().raw().toBuffer({resolveWithObject:true});
 decodedImages.push({encoded_bytes:input.length,width:info.width,height:info.height,channels:info.channels,rgba_bytes:data.length});
...
const names=process.argv.slice(4);if(!names.length)names.push('blockling','signal-moth','buffer-baron');
const specs={'blockling':[.85,0,null],'signal-moth':[.90,.12,1.25],'buffer-baron':[1.85,.10,null]};
let failed=false;
for(const name of names){
 ...
 const [h,f,w]=specs[name];const checks={embedded_image_decoded:decodedImages.length>0&&decodedImages.every(i=>i.width===1024&&i.height===1024),six_or_fewer_draw_primitives:skinned.length<=6,skinned_geometry_loaded:skinned.length>0,
 ...
 const record={asset_id:name,tool:'Three GLTFLoader + AnimationMixer + actual sampled skinned vertices + precise bounds',three_revision:THREE.REVISION,
  ...
  bones:skinned[0].skeleton.bones.map(b=>b.name),decoded_images:decodedImages.slice(),animations,checks,limitations:'CPU loader, real image decode and geometry evidence; no WebGL raster, physical Safari or frame-time claim.'};
