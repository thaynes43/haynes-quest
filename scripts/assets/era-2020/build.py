...
Carved mesh relief and original baked pigment; one embedded atlas per model.
No external meshes, image downloads, private inputs, or special decoders.
...
def leaf(name,start,end,width,mat='Sage moss',bone='body',tint=1,bulge=.009,steps=7):
 ...
 tube(name+' | midrib',[a+d*t+Vector((0,bulge*1.52*math.sin(math.pi*t)+.001,0)) for t in [j/(steps-1) for j in range(steps)]],.0018,mat,bone,tint*1.26,4)
 return ob
...
def panel(name,poly,depth,mat,bone='body',tint=1,bevel=.012,relief=.0025,bulge=0,pits=()):
 ...
 if name.startswith('Face | carved smiling'):nu=18;nv=13
 verts=[];faces=[]
 for j in range(nv):
...
def blockling():
 ...
 # A single shallow branched edge split complements the baked plank seams.
...
def halo_segment(index,angle,bn):
 center=Vector((0,-.025,1.546));r0=.355;r1=.452;span=.74;steps=8
 rr=[]
 ...
 for sign in [-1,1]:
  ...
  tube('Halo | engraved scroll '+str(index)+' '+str(sign),pts,.0022,'Walnut',bn,.91,3)
 # A continuous warm raised border follows the segment's two arcs.
 outline=[]
 for rradius,sequence in [(.442,range(6)),(.367,reversed(range(6)))]:
  for j in sequence:
   a=angle-span*.44+span*.88*j/5;outline.append((rradius*math.cos(a),.021,center.z+rradius*math.sin(a)))
 tube('Halo | beveled border '+str(index),outline,.0028,'Honey brass',bn,1.22,3,True)
...
def buffer_baron():
 ...
 for z,rx,ry in profile:
  ringpts=[]
  for i in range(28):
   a=TAU*i/28;groove=.005*math.cos(a*10+.18*math.sin(z*4))
 ...
 ball('Head | rounded walnut cranium',(0,-.018,1.544),(.258,.155,.277),'Walnut','head',.86,n=24,k=10,ribs=.014)
 ...
 for s in [-1,1]:
  ...
  ball('Face | amber eye lens '+str(s),(x,.211,z),(.031,.031,.037),'Amber light','head',.70,16,8)
 ...
 leaf('Face | central carved bridge',(0,.21,1.446),(0,.185,1.779),.033,'Walnut','head',1.12,.012,steps=6)
 ...
 XFORM=Matrix.Translation((0,.10,1.546))@Euler((.26,0,0)).to_matrix().to_4x4()@Matrix.Translation((0,0,-1.546))
 halo_frame=XFORM.copy()
 for i in range(6):
  XFORM=Matrix.Translation((-.23 if i==4 else .23,0,.20))@halo_frame if i in [4,5] else halo_frame
  halo_segment(i,TAU*i/6,'halo')
 XFORM=Matrix.Identity(4)
 ...
 ball('Chest | amber glass',(0,.266,.984),(.132,.045,.145),'Amber light',tint=.87,n=24,k=9)
 for i in range(4):
  a=TAU*i/4;points=[]
  for j in range(6):
 ...
 for s in [-1,1]:
  ...
  for j,p in enumerate(centers):
   r=[.048,.100,.120,.112,.090,.061][j]
   rr.append([(p.x+r*math.cos(TAU*i/18),p.y+r*.83*math.sin(TAU*i/18),p.z) for i in range(18)])
  ...
  ball('Hand | carved palm '+side,palm,(.084,.069,.090),'Walnut','hand.'+side,1.06,n=16,k=8)
  for j in range(2):
   ...
   tube('Hand | curled finger '+bn,points,[.029,.031,.029,.021],'Walnut',bn,1.09,9)
  ...
  for j in range(3):
   leaf('Mantle | shoulder leaf '+side+' '+str(j),(s*(.18+j*.018),.17+j*.023,1.245),(s*(.43+j*.019),.10+j*.040,1.255-j*.047),.048,bone='body',tint=.82+j*.08,bulge=.025,steps=6)
  tube('Mantle | curled vine '+side,[(s*.181,.185,1.25),(s*.207,.24,1.17),(s*.206,.271,1.10),(s*.232,.246,1.05)],.009,'Sage moss',tint=.91,sides=6)
  for j in range(3):
   leaf('Mantle | chest leaf '+side+' '+str(j),(s*.207,.253,1.17-j*.054),(s*(.255 if j%2==0 else .153),.273,1.226-j*.054),.025,bone='body',tint=.91,bulge=.011,steps=6)
  leaf('Gauntlet | outer leaf '+side,(s*.463,.13,.752),(s*.484,.125,.936),.05,bone='forearm.'+side,tint=.82,bulge=.015,steps=6)
...
if __name__=='__main__':
 ...
 parser.add_argument('--name',required=True,choices=list(SPECS))
 parser.add_argument('--output',default=OUTPUT)
 args=parser.parse_args(sys.argv[sys.argv.index('--')+1:] if '--' in sys.argv else [])
 build(args.name,args.output)
