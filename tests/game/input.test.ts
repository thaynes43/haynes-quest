...
import { bindBrowserInput, GameInputState, getJoystickVector } from '../../src/game/input';
...
class FakeEventTarget {
  ...
  dispatch(type: string, event: Record<string, unknown> = {}): void {
    for (const listener of this.listeners.get(type) ?? []) {
      if (typeof listener === 'function') listener({ type, ...event } as unknown as Event);
...
describe('game input', () => {
  ...
  it('clears independent channels on blur and pointer cancellation and removes listeners on dispose', () => {
    ...
    fakeTarget.getBoundingClientRect = () => ({ left: 0, width: 100 } as DOMRect);
  ...
  it('caps joystick travel and maps upward screen travel to forward movement', () => {
    expect(getJoystickVector(50, 50, 80, 10, 25)).toMatchObject({ distance: 1 });
