...
/**
 * WO074 probe: the "popup when attacking beside a keepsake" report. A minor
 * memory pickup shows a brief picture inside the attack notice. Any other
 * notice within that 2.3 s window cancels the pickup timer without clearing
 * pickupMemoryId, so every later attack notice carries the stale picture.
 */
...
describe("WO074 attack-notice picture lifecycle (717cfe6)", () => {
  it("does not resurrect the collected minor's picture on later attack notices", async () => {
    ...
    const pictureDuringWindow = Boolean(notice()?.querySelector("img"));
    ...
    console.log(
      `WO074 notice picture: shown during the pickup window=${pictureDuringWindow}; 30 s later a plain attack notice still carries the keepsake picture=${stalePicture !== null && stalePicture !== undefined}`,
    );
    expect(notice()?.textContent).toContain("Move closer to a glowing enemy");
